...
for iH = 1:numel(sph)   
    ...
    if isfield(cfg,'deg_fre')
        pvl_grp_low = tinv(10^-fdr_hld(iH),cfg.deg_fre);
        pvl_grp_hgh = tinv(1-10^-fdr_hld(iH),cfg.deg_fre);
        
        mem_grp = cfg.plt_dta{iH};
        mem_grp( mem_grp >  pvl_grp_low & mem_grp <  pvl_grp_hgh ) = 0;
        mem_grp( mem_grp <= pvl_grp_low | mem_grp >= pvl_grp_hgh ) = mem_grp(mem_grp <= pvl_grp_low | mem_grp >= pvl_grp_hgh) ./ max_val;
        mem_grp( mem_grp <= -1) = -0.99;
        mem_grp( mem_grp >= 1)  = 0.99;
        mem_grp = (mem_grp + 1) / 2;
    elseif isfield(cfg,'fmr_rng_num') && ~isempty(cfg.fmr_rng_num)
        pvl_grp_low = cfg.fmr_rng_num(1);
        pvl_grp_hgh = cfg.fmr_rng_num(2);
        
        mem_grp = cfg.plt_dta{iH};
        mem_grp = mem_grp ./ cfg.fmr_rng_num(2);
        mem_grp( mem_grp <= -1) = -0.99;
        mem_grp( mem_grp >= 1)  = 0.99;
        mem_grp = (mem_grp + 1) / 2;
...
clb.TickLength = 0;
clb.TickLabels = cellfun(@num2str,num2cell(roundsd(linspace(cfg.fmr_rng_num(1),cfg.fmr_rng_num(2),11),2)),'uni',0);
