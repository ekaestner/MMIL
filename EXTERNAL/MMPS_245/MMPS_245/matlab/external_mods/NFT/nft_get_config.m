...
%conf.nft_dir = mfiledir(1:max(find(mfiledir == filesep) - 1));

% set nft directory (DH 02/19/2014)
conf.nft_dir = [getenv('MMPS_EXT') '/NFT-2.3_64bit'];

% Add bin
bindir = [conf.nft_dir filesep 'bin' filesep];
if ispc
    sufx = '.exe';
else
    sufx = '';
