function      [data, result]= readtext_jcr(fname, delimiter, comment, quotes, options)

%  Usage:     [data, result]= readtext_jcr(fname, delimiter, comment, quotes, options)
% 
...
%            delimvec = zeros(1,length(linestr));
%            delimvec(regexp(linestr,delimiter)) = 1;
%            delimvec = delimvec & ~quotedvec;
          ...
          lp1 = eolindvec(n)+1;
%          if mod(n,10000)==0, fprintf(1,'n=%6d of %6d (%6.2f%%)\n',n,nrows,100*n/nrows); end
        end
        ncols = max(ncolvec);
        ...
        for n = 1:nrows
          ...
          lp1 = eolindvec(n)+1;
%          if mod(n,10000)==0, fprintf(STD_ERR,'n=%6d of %6d (%6.2f%%)\n',n,nrows,100*n/nrows); end
        end
        if STD_ERR ~= 1, fclose(STD_ERR); end
