function results = MMIL_Summarize_rsBOLD_Analysis(ProjID,varargin)
%function results = MMIL_Summarize_rsBOLD_Analysis(ProjID,[options])
%
% Usage:
%  results = MMIL_Summarize_rsBOLD_Analysis(ProjID,'key1', value1,...);
%
% Required Input:
...
%     {default = 0.5}
%   'roi_type': type of surface ROI to be used ('fparc', 'talrc', or 'aparc')
%     {default = 'fparc'}
...
% Prev Mod: 04/23/17 by Don Hagler
...
... % other
    ...
    'networks',[],[],...
    'roinames',[],[],...
    'roi_type','fparc',{'fparc','talrc','aparc'},...
    'verbose',1,[0:2],...
...
...
    ...
    'results_tags',{'StudyInfo','SubjIDs','VisitIDs','STRUCT_VisitIDs',...
                   'VisitNumbers','TR','nreps','numTRs'},[],...
    'motion_tags',{'mean_motion','max_motion',...
...
...
  ...
  if isempty(parms.networks)
    parms.networks = mmil_define_rsBOLD_networks(parms.roi_type);
  end;
