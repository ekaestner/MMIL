function [d, snr_d] = mux_epi_main(pfile, outfile, ext_cal, slices, nt_to_recon, n_vcoils, debug, recon_method, apply_fermi, use_homodyne, notch_thresh, dcmfile, dataFileHeader, maxim, new_se_uid, outdir)
%
% [d, snr_d] = mux_epi_main(pfile, [outfile=[]], [ext_cal=[]], [slices=[]], [nt_to_recon=[]], [n_vcoils=[]], [debug=false], [recon_method='1Dgrappa'], [apply_fermi=false], [use_homodyne=true], [notch_thresh=0])
%
...
npass       = dataFileHeader.rdb_hdr.npasses;
cal_phases  = p.num_mux_cycle*p.mux;
nslices     = dataFileHeader.rdb_hdr.nslices/dataFileHeader.rdb_hdr.reps;
slice_thk   = dataFileHeader.image.slthick;
slice_sep   = dataFileHeader.rdb_hdr.user8;
...
for t = 1:size(d,4)
    for sl = 1:size(d,3)
        d_(:,:,sl,t) = imrotate(d(:,:,sl,t),dataFileHeader.rdb_hdr.rotation*90);
    end
end
...
if maxim > 0
    ...
    for sl_ind = 1:p.mux
        write_dicoms(real(squeeze(d_(:,:,sl_ind,2:end))), sl_loc(sl_ind), ...
            slice_demux_coord(sl_loc(sl_ind)), dataFileHeader, t_array, ...
            num_t2, dcmfile, maxim, new_se_uid, outdir);
    end
end
