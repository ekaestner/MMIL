function N = ReadECATfile(fname,opts)
% Import ECAT 7 images from CTI PET scanners.
% FORMAT N = ECATread(fname)
%  fname     - name of ECAT file
% _______________________________________________________________________
% Copyright (C) 2005 Wellcome Department of Imaging Neuroscience, UCSD

% John Ashburner & Roger Gunn, modified by Anders M. Dale
...
%    dato     = file_array(fnameo,dim,[4 spm_platform('bigend')],0,scale,inter);
    ...
    M = M(:,[2 1 3 4]);
    N{i}.filetype = 'ECAT7';
    N{i}.PatientID = mh.PATIENT_ID;
%    e7datenumoffset = datenum(2006,5,30,12,01,39)-1149015699/(3600*24); % Calculate offset in secs for date conversion
    e7datenumoffset = datenum(2006,3,20,12,54,21)-1142888061/(3600*24); % Calculate offset in secs for date conversion
    N{i}.StudyDateStr = datestr(mh.SCAN_START_TIME/(3600*24)+e7datenumoffset);
    N{i}.StudyDateNum = mh.SCAN_START_TIME/(3600*24)+e7datenumoffset;

    N{i}.vol = vol;
    N{i}.M = M;

%    N{i}.descrip     = sh(i).ANNOTATION;
    N{i}.timing = struct('toffset',sh(i).FRAME_START_TIME/1000,'tspace',sh(i).FRAME_DURATION/1000);
    N{i}.extras = struct('mh',mh,'sh',sh(i));
end;
return;
%_______________________________________________________________________
