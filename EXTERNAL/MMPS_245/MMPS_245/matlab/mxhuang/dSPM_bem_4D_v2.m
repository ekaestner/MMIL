...
INPUT = 'dSPM_bem_4D_v2_default_input';	% file to get default inputs
TEMP = 'dSPM_bem_4D_v2_temp_output';	% file to write this run's inputs
...
if strcmp(datafile(length_datafile-3:length_datafile),'.fif') == 1
  ...
  [Coil_4D,intpnt_loc_4D,intpnt_ori_4D]=sensor_4D_magnes_WH_normal_mne(fif_data);
  F_orig=F_orig(1:length(Coil_4D),:); % only the MEG data
else
...
% if projection with SSP is needed
% T_project=[];
% if flag_project==1,
%     [UF,SF,VF]=svd(F_baseline);
%     T_project=eye(size(UF,1))-UF(:,1:3)*UF(:,1:3)'; % pick up the largest 3 !!!!!!
%     F=T_project*F; % projection
%     F_baseline=F(:,base_lat(1):base_lat(2)); % reget the baseline after the projection
% else 
    T_project=eye(size(F_orig,1),size(F_orig,1));
%end    

while 1
    cov_method= default_v('Select 1 if covariance matrix is from diagnal element; 2 if full covariance matrix is used : ',f_default,f_temp);
    if cov_method==1,
        Cvar=diag(std(F_baseline').^2);  % diag covariance matrix
        navg=1;
        break;
    elseif cov_method==2,
        Cvar=cov(F_baseline');  % full diag covariance matrix
        navg=1;
        break;
    end
end    
...
G_bem_xyz=T_project*gain_intpnt2chan(Coil_4D,G_bem_xyz_inp.meg);
