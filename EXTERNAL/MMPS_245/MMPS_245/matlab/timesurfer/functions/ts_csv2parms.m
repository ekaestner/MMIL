...
for row = 1:size(data,1)
  ...
  if isstr(dat{1}) && ~any(regexp(dat{1},'\W')) &&...
    ...
    keep = [keep row];
  elseif isstr(dat{1}) && ~any(regexp(dat{1},'\W')) && all(ismember(dat{2},CONTROL_OPTIONS))
    cidx = [cidx row];
  end
end
crows = data(cidx,:);
data  = data(keep,1:3);
toss  = setdiff(1:size(rawdata,1),keep);
other = rawdata(toss,:);
