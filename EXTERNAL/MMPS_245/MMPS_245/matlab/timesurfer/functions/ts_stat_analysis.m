...
opt = mmil_args2parms(varargin,...
                      {'f_path'     , [],[],...
                       ...
                       'blc','no',{'yes', 'no'},...
                       'blcwindow',[],[],...
											 'blcorrection',[],{'no','absolute','relative','relchange','zscore','zscores','yes',''},...
											 'bl_window',[],[],...
											 'bl_tile',0,{0,1},...
...
if isempty(opt.blcorrection), opt=rmfield(opt,'blcorrection'); end
...
if timefreq_flag && strcmpi(opt.method,'montecarlo')
	...
	for i=1:length(f_names)
		...
		fprintf('%s: loading file %d of %d: %s\n',mfilename,i,length(f_names),tmp_str);
		load(tmp_str); % timefreq_data
		data(i) = timefreq_data;
		tossfields = {'power','cmplx','data'};
		toss = find(isfield(data(1).timefreq,tossfields));
		try data(i).timefreq = rmfield(data(i).timefreq,tossfields{toss}); end
		clear timefreq_data;
	end

	if (isstr(opt.channel) && strcmpi(opt.channel,'all'))
  	opt.channel = 1:data(1).num_sensors;
	end
	
	% Check channel selections
  if (~isempty(opt.chantype) && ~isempty(opt.channel))
    %mmil_error(opt, 'Cannot specify BOTH chantype AND channels param values.');
		% keep channel
		opt.chantype = [];
	end
	...
	for ch = 1:length(channels)
	  ...
	  for k=1:length(events),
	    ...
	    infile = sprintf('%s%d/%s',f_path,events(k),infile);
			fprintf('%s: loading file %d of %d: %s\n',mfilename,k,length(events),infile);
	    load(infile); 
		...
		fprintf('%s: analyzing channel %d: %d of %d\n',mfilename,channels(ch),ch,length(channels));
