function errcode = BOLD_MMIL_Resting_Analysis(ContainerPath,FSContainerPath,varargin)
%function errcode = BOLD_MMIL_Resting_Analysis(ContainerPath,FSContainerPath,[options])
%
...
%
% Usage:
%  BOLD_MMIL_Resting_Analysis(ContainerPath,FSContainerPath,'key1', value1,...)
%  e.g. BOLD_MMIL_Resting_Analysis(ContainerPath,FSContainerPath,...
...
% Prev Mod: 04/13/17 by Don Hagler
...
%% todo: exclude frames with excessive motion?
...
function [parms,errcode] = locate_files(parms)
  ...
  for i=1:parms.nscans
    % need to know number of TRs, but don't want to spend time
    %   untarring mgz files if everything is done already
    fname_info = sprintf('%s_data_scan%d_info.mat',...
      parms.outstem,i);
    if ~exist(fname_info,'file') || parms.forceflag
      mmil_mkdir(parms.outdir);
      [M,volsz] = fs_read_header(parms.fnames_BOLD{i});
      save(fname_info,'M','volsz');
    else
      load(fname_info);
    end;
...
function parms = extract_group_tseries(parms,group_name)
  ...
  for i=1:parms.nscans
    ...
    if ~exist(fname_tseries,'file') || parms.forceflag
      if parms.verbose==2
        fprintf('%s: extracting white matter time series for scan %d...\n',...
          mfilename,i);
      end;
...
function parms = prep_data(parms)
  ...
  if parms.mask_flag ||...
    ...
    for i=1:parms.nscans
      fname_BOLD = parms.fnames_BOLD{i};
      [tmp,fstem] = fileparts(fname_BOLD);
      fname_BOLD_prep = sprintf('%s_data_scan%d_prep.mgz',...
        parms.outstem,i);
      if ~exist(fname_BOLD_prep,'file') || parms.forceflag
        ...
        if parms.mc_flag==2
          if parms.deriv_flag
            parms.motion_data = mmil_load_motion_1D(parms.fnames_motion{i},...
              'nframes',parms.numTRs(i));
            tmp_parms.regressors = cat(2,tmp_parms.regressors,parms.motion_data);
            motion_deriv = cat(1,zeros(1,6),diff(parms.motion_data));
            tmp_parms.regressors = cat(2,tmp_parms.regressors,motion_deriv);
          else
            tmp_parms.fname_motion = parms.fnames_motion{i};
          end;
        end;
...
function parms = concat_data(parms)
  ...
  if parms.mc_flag
    ...
    if ~exist(parms.fname_motion,'file') || parms.forceflag
      ...
      for i=1:parms.nscans
        tmp_data = mmil_load_motion_1D(parms.fnames_motion{i},...
        ...
        if parms.mc_bandpass_flag && parms.bandpass_flag
          tmp_data = ts_freq_filt(tmp_data,1/parms.TRs(i),...
      ...
      save(parms.fname_motion,'motion_data',...
...
function parms = extract_surf_tseries(parms)
  parms.fname_surf_data = sprintf('%s_surf_roi_data.mat',parms.roi_outstem);
...
function parms = extract_vol_tseries(parms)
  parms.fname_vol_data = sprintf('%s_vol_roi_data.mat',parms.roi_outstem);
...
function parms = combine_roi_tseries(parms)
  parms.fname_roi_data = sprintf('%s_roi_data.mat',parms.roi_outstem);
...
function [timevec,timestrs] = set_time(parms,ntpoints)
  timevec = []; timestrs = [];
  ntpoints_data = sum(parms.numTRs) - (parms.nscans * parms.skipTRs);
  if ntpoints_data ~= ntpoints
    error('sum of numTRs (%d) minus total number of skipped TRs (%d) does not match number of time points (%d)',...
      sum(parms.numTRs),parms.nscans * parms.skipTRs,ntpoints);
  end;
  ...
  for i=1:parms.nscans
    TR = parms.TRs(i);
    numTRs = parms.numTRs(i) - parms.skipTRs;
    for r=1:numTRs
...
function parms = write_csv_tseries(parms,timestrs)
  parms.fname_csv_tseries = sprintf('%s_roi_tseries.csv',parms.roi_outstem);
...
%  tmp_parms.verbose = (parms.verbose==2);
  ...
  for h=1:parms.nhemi
    ...
    tmp_parms.fname_out = sprintf('%s-sm%d-ico%d-%s.%s',...
      parms.outstem,parms.ico_presmooth,tmp_parms.icolevel,...
...
function parms = calc_variance(parms)
  ...
  if parms.aparc_flag || parms.aseg_flag
    fname_out = sprintf('%s_roi_data_var.mat',parms.roi_outstem);
    if ~exist(fname_out,'file') || parms.forceflag
      [roi_data,roinames] = get_roi_data(parms);
      roi_var = var(roi_data,1,1);
      save(fname_out,'roi_var','roinames','-v7.3');
    end;
  end;
  ...
  if parms.ico_flag
    for h=1:parms.nhemi
      hemi = parms.hemilist{h};
      fname_out = sprintf('%s-sm%d-ico%d-var-%s.%s',...
        parms.outstem,parms.ico_presmooth,parms.ico_order,hemi,...
        parms.outtype);
      if ~exist(fname_out,'file') || parms.forceflag
...
function parms = calc_correlations(parms)
  ...
  if parms.corr_roi_flag && (parms.aparc_flag || parms.aseg_flag)
    parms.fname_roi_corr = sprintf('%s_roi_corr.mat',parms.corr_outstem);
    if ~exist(parms.fname_roi_corr,'file') || parms.forceflag
      ...
      save(parms.fname_roi_corr,...
        'seed_data','seed_roinames','seed_roinames_full',...
        'roi_data','roinames','time','R','Z','-v7.3');
    end;
    % plot correlation image
    fname_plot = sprintf('%s_roi_corr.tif',parms.corr_outstem);
  ...
  if parms.corr_ico_flag && parms.ico_flag
    parms.fname_ico_corr = sprintf('%s_ico%d_corr.mat',...
      parms.corr_outstem,parms.ico_order);
    if ~exist(parms.fname_ico_corr,'file') || parms.forceflag
      ...
      save(parms.fname_ico_corr,...
        'ico_data','hemis','vertices','time','R','Z','-v7.3');
    end;  
  end;
  ...
  if parms.corr_roi_ico_flag && parms.ico_flag &&...
    ...
    parms.fname_roi_ico_corr = sprintf('%s_roi_ico%d_corr.mat',...
      parms.corr_outstem,parms.ico_order);
    ...
    if parms.corr_roi_ico_maps_flag
      ...
      for r=1:length(seed_roinames)
        ...
        for h=1:length(parms.hemilist)
          hemi = parms.hemilist{h};
          fname = sprintf('%s_%s_ico%d_corr-%s.mgz',...
            parms.corr_outstem,seed_roiname,parms.ico_order,hemi);
      ...
      if ~all_exist
        ...
        for r=1:length(seed_roinames)
          ...
          for h=1:length(parms.hemilist)
            hemi = parms.hemilist{h};
            fname = sprintf('%s_%s_ico%d_corr-%s.%s',...
              parms.corr_outstem,seed_roiname,parms.ico_order,hemi,...
              parms.outtype);
            if ~exist(fname,'file') || parms.forceflag
              ind = find(hemis==h);
...
function [data,roinames,time] = get_roi_data(parms)
  ...
  if 0
    % exclude ROIs with NaNs or all zeros
    mean_data = mean(data,1);
    ind_good_rois = find(~isnan(mean_data) & mean_data~=0);
    data = data(:,ind_good_rois);
    roinames = roinames(ind_good_rois);
  end;
return;
