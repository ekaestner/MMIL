/* normbrik.c: read brik, normalize by standard deviation, write brik
      created: 02/08/06 DH
     last mod: 02/08/06 DH

   purpose:
     normalizing brik by its standard deviation
...
void usage()
{
  ...
  printf("    -imgindex [0]              sub-BRIK index to normalize\n");
  printf("    -indir    [.]              input dir\n");
  printf("    -outstem  [norm-instem] output file stem: <outstem>.BRIK\n");
...
int main(int argc, char **argv)
{
  ...
  /* calculate variance of voxels inside the mask */
  for(k=0;k<nvox_m;k++) {
    i = maskvox[k];
    f = dat[i];
    sum += f;
  ...
  avg = sum/nvox_m;
  stdev = sqrt(sum2/nvox_m-avg*avg);
  MsgPrintf("%s: calculated stdev = %e\n",progname,stdev);

  /* normalize all voxels by stdev */
  for(i=0;i<nvox;i++) {
    dat[i] = (10^6)*dat[i]/stdev;
  }
