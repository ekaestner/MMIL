...
//
// Usage:   fact  ParaFile <enter>
//
//			fact			-- the program name
//			ParaFile		-- parameter file
//
// Return:  0, if successful. otherwise, an error code defined in "fact.h"
//
// Author:  Hangyi Jiang
//          Radiology Dept. MRI Group
//			Johns Hopkins University, School of Medicine
//			Baltimore, MD 21205
//
// Date:	July, 2005
//
// Note:	Although C++ is used, it is basically a C program, in case that the platform does not
//			have a C++ compiler. 
//
// Update:
//
//  Modified by Sumiko Abe, MMIL, UCSD to take mgh files as input
//  Last Modified by Don Hagler, MMIL, UCSD  04/19/2008
//  Last Modified by Don Hagler, MMIL, UCSD  04/13/2009
//================================================================================================
...
{
	printf("fact: Fiber Assignment by Continuous Tracking -- fiber tracking toolkit\n");
	printf(" By Hangyi Jiang, July 2005, a hot summer\n");
	//-----------------------------------------------------------
	// checking: command line parameter
	//-----------------------------------------------------------
	//if (argc < 13) { DispErrorMsg(DM_ERR_Usage); return DM_ERR_Usage; }
	if (argc < 3 || argv[1][0] != '-' || strcmp(argv[1], "-help")  == 0)
	{ 
	...
	//-----------------------------------------------------------
	// initialize dpf file structure

	//-----------------------------------------------------------
	// Fiber tracking
	//-----------------------------------------------------------
	printf("fiber tracking....\n");
	...
	if( (option_st[0].optionflag == 1) )
	{
		int ret = getFactPara(argc, argv,  stFactMap.pstFactPara);
		ReadImgFiles(&stFactMap);
	}
	else if( (option_st[19].optionflag == 1) && (option_st[20].optionflag == 1))
	{
		//Read Raw data
		if( (option_st[21].optionflag == 0) || (option_st[24].optionflag == 0) || ((option_st[22].optionflag == 0) && (option_st[23].optionflag == 0)) )
		{
			cout << " Parameter input error: check image size, voxel size, FOV size and image orientation input." << endl;
			return -1;
		}
		int ret = getFactPara(argc, argv,  stFactMap.pstFactPara);
		ReadImgFiles(&stFactMap);
	}
	else if( (option_st[1].optionflag == 1) && (option_st[2].optionflag == 1))
	{
		//Read mgh data
		int ret = getFactPara(argc, argv,  stFactMap.pstFactPara);
		ReadMghImgFiles(&stFactMap);
	}
	...
	cout << "nFiberLenMin = " << stFactMap.pstFactPara->nFiberLenMin<< endl;
	cout << "nFiberLenMax = " << stFactMap.pstFactPara->nFiberLenMax<< endl;
	cout << "============================================== "<< endl;

	//Tracking from rois
	//nRetCode = RoiFiberTrackingFACT(&stFactMap, roi_info);

	//Tracking whole brain
	int nRetCode = FiberTrackingFACT(&stFactMap);

	printf("# of Fibers = %d\n", stFactMap.lFiberNrTotal);

	//-----------------------------------------------------------
	// save all fibers
	//-----------------------------------------------------------
	if (stFactMap.pstFactPara->szFiberAllFile != NULL) {
		printf("save all fibers as a data file....\n");
		FiberSave(&stFactMap, false);			// save all fibers
	};

	//-----------------------------------------------------------
	// save all fibers as a plain text file
	//-----------------------------------------------------------
	if (stFactMap.pstFactPara->szFiberAllTxtFile != NULL) {
		printf("save all fibers as a plain-text file....\n");
		FiberSaveTxt(&stFactMap, false);			// save all fibers
	}

	//-----------------------------------------------------------
	// testing: should we do fiber selection?
	//-----------------------------------------------------------
	if (stFactMap.pstFactPara->szBinRoiFile[0] == NULL) {		// no binary ROI files. simply return
		FreeMemFactMap (&stFactMap);
		FreeMemFactPara (stFactMap.pstFactPara);
		printf("\nFinished, thank you for using program.\n");
		return DM_PROC_Ok;
	}

	//-----------------------------------------------------------
	// create index array for fiber selection
	//-----------------------------------------------------------
	printf("create fiber index array....\n");
	nRetCode = CreateFiberIndexAry(&stFactMap);
	...
	//-----------------------------------------------------------
	// do fiber selection via ROI files
	//-----------------------------------------------------------
	printf("fiber selecting using ROI files....\n");
	nRetCode = FiberSelectByRoiMap(&stFactMap);
	if (nRetCode != DM_PROC_Ok) {
		FreeMemFactMap (&stFactMap);
		FreeMemFactPara (stFactMap.pstFactPara);
		return nRetCode;
	...
	//-----------------------------------------------------------
	// save selected fibers
	//-----------------------------------------------------------
	if (stFactMap.pstFactPara->szFiberSelFile != NULL) {
		printf("save selected fibers as a data file....\n");
		FiberSave(&stFactMap, true);			// save select fibers
	}

	//-----------------------------------------------------------
	// save selected fibers as a plain text file
	//-----------------------------------------------------------
	if (stFactMap.pstFactPara->szFiberSelTxtFile != NULL) {
		printf("save selected fibers as a plain-text file....\n");
		FiberSaveTxt(&stFactMap, true);			// save select fibers
	}

	/*	*/
	if (stFactMap.pstFactPara->szFiberSelMghFile != NULL){
		mgh_header fiberVolum;
		printf("save selected fibers as a mgh format file....\n");
		int res = Fiber2Mgh(stFactMap, &fiberVolum);
		if(res == -1)
		{
			cout << "mgh file output error: " << stFactMap.pstFactPara->szFiberSelMghFile << endl;
			return -1;
		}
		writeMgh(stFactMap.pstFactPara->szFiberSelMghFile, &fiberVolum, 1);
	}
	if (stFactMap.pstFactPara->szFiberSelVolFile != NULL){
		writeRaw(stFactMap);
	}
	/*	*/
	...
	FreeMemFactMap (&stFactMap);
	FreeMemFactPara (stFactMap.pstFactPara);
	printf("\nFinished, thank you for using this program.\n");
	return 0;
}
...
bool DispErrorMsg(int DM_ERR_Code, void* pPara0, void* pPara1, void* pPara2, void* pPara3)
{
    ...
    cout << " Program Usage: factnew -options option_parameters\n" << endl;
    cout << " Option Usages: " << endl;
    cout << "  -help:              print out usages of this program"<< endl;
    cout << "  -iParatxt:          input a text parameter file defined by Mori's format"<< endl;
    cout << "  -iFAmgh:            input a mgh format FA image" << endl;
    cout << "  -iV0mgh:            input a mgh format V0 image" << endl;
    cout << "  -iStartFA:          input threshold of start FA  for tracking" << endl;
    cout << "  -iStopFA:           input threshold of stop FA value for tracking" << endl;
    cout << "  -iTurnAngleDeg:     input threshold of trun angle degree for tracking"<< endl;
    cout << "  -iFlipEigenX:       flip eigenvector in x direction Example: -iFlipEigenX 1"<< endl;
    cout << "  -iFlipEigenY:       flip eigenvector in y direction"<< endl;
    cout << "  -iFlipEigenZ:       flip eigenvector in z direction"<< endl;
    cout << "  -iRoimghOR:         OR operation input roi mgh format file"<< endl;
    cout << "  -iRoimghAND:        AND operation ROI list input follow by ROI files list"<< endl;
    cout << "  -iRoimghNOT:        NOT operation ROI list input follow by ROI files list"<< endl;
    cout << "  -iRoimghCUT_OR:     CUT_OR operation ROI list input follow by ROI files list"<< endl;
    cout << "  -iRoimghCUT_AND:    CUT_AND operation ROI list input follow by ROI files list"<< endl;
    cout << "  -oAllFiberFile:     output bin file name of all fiber result"<< endl;
    cout << "  -oSelFiberFile:     output bin file name of fiber result selected by ROIs"<< endl;
    cout << "  -oAllFiberTxtFile:  output txt file name of all fiber result"<< endl;
    cout << "  -oSelFiberTxtFile:  output txt file name of fiber result selected by ROIs"<< endl;
    cout << "  -oSelFiberMghFile:  output bin file name of fiber result selected by ROIs"<< endl;
    cout << "  -oSelFiberVolFile:  output bin file name of fiber result selected by ROIs"<< endl;
    cout << "  -iFAraw:            input bin FA file name"<< endl;
    cout << "  -iV0raw:            input bin V0 file name"<< endl;
    cout << "  -iImgDims:          input image matrix size Example: -iImgDims 128 128 47"<< endl;
    cout << "  -iVoxSize:          input image voxel size Example: -iVoxSize 1.875 1.875"<< endl;
    cout << "  -iSliceThickness:   input image slice thickness Example: -iSliceThickness 5.0"<< endl;
    cout << "  -iFOVSize:          input image FOV size Example: -iFOVSize 240 240"<< endl;
    cout << "  -iImageOrientation: input image orientation: 0 coronal, 1 axial, 2 saggital"<< endl;
    cout << "  -iRoirawOR          OR operation input raw roi image"<< endl;
    cout << "  -iRoirawAND         AND operation input raw roi image"<< endl;
    cout << "  -iRoirawNOT         NOT operation input raw roi image"<< endl;
    cout << "  -iRoirawCUT_OR      CUT_OR operation input raw roi image"<< endl;
    cout << "  -iRoirawCUT_AND     CUT_AND operation input raw roi image"<< endl;
    cout << "  -iByteSwapFlag      byte swap flag: 0 non byteswap, 1 byteswap"<< endl;
    cout << "  -iFiberLenMin       minimum length of tracking fiber"<< endl;
    cout << "  -iFiberLenMax       maximum length of tracking fiber"<< endl;
    cout << endl;
...
int ReadFactParaFile(char* szParaFileName, FACT_PARA* pstFactPara)
{
	...
	while (!bEof) {
		...
		else if (strcmp(szTmpBuf,"MaximumFiberLen:")==0)   sscanf(szLnBuf,"%s %d\n", szTmpBuf, &(pstFactPara->nFiberLenMax));	//maximum fiber length
...
**/
	...
	// otherwise, the image pixel is actually located in non-interger position that makes 
...
int RoiFiberTrackingFACT(FACT_MAP*	pstFactMap, roiInfo RoiInfo)
{
	...
	int	nFiberLenMax = pstFactMap->pstFactPara->nFiberLenMax;
	...
	for (int i=0; i<RoiInfo.ROI_size; i++) 
	{
		...
		XYZ_TRIPLE* pxyzFiberChain = GetFiberChainFact(pfImgFa, pxyzImgVec, 
						...
						xyzSeed, fStopFA, fTurnDeg, nFiberLenMin, nFiberLenMax,
						&nFiberLen);
...
//        printf("debug: running GetFiberChainFact for [%d,%d,%d]...\n",nX,nY,nZ);
				XYZ_TRIPLE* pxyzFiberChain = GetFiberChainFact(pfImgFa, pxyzImgVec, 
										nImgWidth, nImgHeight, nImgSlices,
										xyzSeed, fStopFA, fTurnDeg, nFiberLenMin, nFiberLenMax,
										&nFiberLen);
...
//			nFiberLenMax	-- maximum fiber chain length
...
XYZ_TRIPLE*	GetFiberChainFact(float* pfImgFA, XYZ_TRIPLE* pxyzImgVec, 
							  ...
							  int nFiberLenMax,		// maximum length of the fiber chain
							  int* pnFiberLen		// return value
							  )
{
	// forward tracking, get a half-fiber chain
	XYZ_TRIPLE xyzFiberPtF[nFiberLenMax];
	int	nFiberLenF = GetHalfChainFact(pfImgFA, pxyzImgVec,
									  nImgWidth, nImgHeight, nImgSlices,
									  xyzSeedPt,fStopFA, fTurnDeg, nFiberLenMin, nFiberLenMax,
									  1, xyzFiberPtF);

//printf("debug: finished running first half...\n");

	// backward tracking, get a half-fiber chain
	XYZ_TRIPLE	xyzFiberPtB[nFiberLenMax];	// backward tracking result
	int	nFiberLenB = GetHalfChainFact(pfImgFA, pxyzImgVec,
									  nImgWidth, nImgHeight, nImgSlices,
									  xyzSeedPt, fStopFA, fTurnDeg, nFiberLenMin, nFiberLenMax,
									  -1, xyzFiberPtB);

//printf("debug: finished running tracking, now putting fibers together...\n");
					 ...
					 )
{
	...
	while (!bTrackingOK) {
		// some how, the eigenvector may equal to zero, although it is nonsense, it does exist in some
...
//    printf("debug: max fiber len = %d\n",nFiberLenMax);
    if (nFiberChainIdx>=nFiberLenMax-1) {
//      printf("debug: this track is too long, stopping\n");
      bTrackingOK = true;
      continue;
    }
		...
		// cast float to integer coornate and verify its validity
...
int FiberSelectByRoiMap(FACT_MAP*	pstFactMap)
{
	...
	for (int i=0; i<MAX_ROI_FILE_Nr; i++) {
		...
		float*  pByteImg = (float *)malloc(sizeof(float) * lwImgBlkSize);
		if (!pByteImg) {
			DispErrorMsg(DM_ERR_MemAlloc, (void*)szRoiFile[i]);
			return	DM_ERR_MemAlloc;
		}
		memset(pByteImg,0,sizeof(float)*lwImgBlkSize);	// clear memory
		...
		if(nRoiFileType[i] == 0)
		{
			...
			// read file
			if ( fread(pByteImg, sizeof(float),  lwImgBlkSize, inFile ) != (size_t)lwImgBlkSize) {
			...
			if(pstFactMap->pstFactPara->nImgSequencing == 1)
			{
				int k1, k2;
				float *tempChar = pByteImg;
				for(k1=0; k1<nImgSlices; k1++)
					for(k2=0; k2<ns; k2++)
						pByteImg[k1*ns+k2] = tempChar[(nImgSlices-(k1+1))*ns + k2];
			}
		...
		else if(nRoiFileType[i] == 1)
		{
			...
			if(RoiMghInfo.type == 0)
				for(j=0; j<lwImgBlkSize; j++)
					if(RoiMghInfo.charImageArray[j] != 0)
						pByteImg[j] = 1.0;
						//pByteImg[j] = RoiMghInfo.charImageArray[j];
			if(RoiMghInfo.type == 1)
				for(j=0; j<lwImgBlkSize; j++)
					if(RoiMghInfo.intImageArray[j] != 0)
						pByteImg[j] = 1.0;
						//pByteImg[j] = (unsigned char)RoiMghInfo.intImageArray[j];
			if(RoiMghInfo.type == 2) 
				for(j=0; j<lwImgBlkSize; j++)
					if(RoiMghInfo.longImageArray[j] != 0.0)
						pByteImg[j] = 1.0;
						//pByteImg[j] = (unsigned char)RoiMghInfo.longImageArray[j];
			if(RoiMghInfo.type == 3)
				for(j=0; j<lwImgBlkSize; j++)
					pByteImg[j] = RoiMghInfo.floatImageArray[j];
			if(RoiMghInfo.type == 4)
				for(j=0; j<lwImgBlkSize; j++)
					if(RoiMghInfo.shortImageArray[j] != 0)
						pByteImg[j] = 1.0;
						//pByteImg[j] = (unsigned char)RoiMghInfo.shortImageArray[j];

			int k1, k2;
			float *tempChar = (float *)calloc(lwImgBlkSize, sizeof(float));
			for(j=0; j<lwImgBlkSize; j++)
				 tempChar[j] = pByteImg[j];
...
void LabelFiberViaRoiImg(FIBER	stFiberAry[], int nFiberArySize, int*	nFiberIdxAry[], int	nFiberCntAry[], 
						...
						float* pByteImg,						// binary ROI file
	...
	for (i=0; i<pstFactMap->lFiberNrTotal; i++) {
		...
		if (bSaveSel == true)
			fprintf(outFile, "Seed: (%5.1f %5.1f %5.1f)  ROI: (%5.1f %5.1f %5.1f)\n",
						pstFiberAry[i].xyzPtSeed.x+0.5, pstFiberAry[i].xyzPtSeed.y+0.5, pstFiberAry[i].xyzPtSeed.z,
...
int read_mgh(char *mghFileName,  mgh_header *mghInfor, bool endianInfor)
{
	...
	cout << "... reading MGH file: "<< mghFileName << endl ;
...
char** createOptionArray( )
{
	...
	strcpy(optionArray[32], "-iFiberLenMax");
	strcpy(optionArray[33], "-iImgSequencing");
	strcpy(optionArray[34], "-iSliceThickness");
	strcpy(optionArray[35], "-oSelFiberVolFile");
...
int Fiber2Mgh(FACT_MAP stFactMap, mgh_header *mghInfo)
{
	...
	for (i = 0; i<stFactMap.lFiberNrTotal; i++)
	{
		...
		if(stFactMap.pstFiberAry[i].nSelStatus != 0)
		{
			...
			{
				int x = (int)(stFactMap.pstFiberAry[i].pxyzChain[j].x);
				int y = (int)(stFactMap.pstFiberAry[i].pxyzChain[j].y);
				int z = (int)(stFactMap.pstFiberAry[i].pxyzChain[j].z);
				int location = x + y*mghInfo->width + ((mghInfo->depth-(z+1))*ns);
...
int getFactPara(int argc, char* argv[], FACT_PARA *pstFactPara )
{
	...
	pstFactPara->fStartFA = DEFAULT_STARTFA;			// tracking threshold values
	pstFactPara->fStopFA = DEFAULT_STOPFA;
	pstFactPara->fTurnDeg = DEFAULT_TURNDEG;
	pstFactPara->nFiberLenMin = DEFAULT_FIBERMINLEN;
	pstFactPara->nFiberLenMax = DEFAULT_FIBERMAXLEN;
	...
	for(i=0; i<argc; i++)
	{
		...
		else if (strcmp(argv[i],"-iFiberLenMax")==0)   
			pstFactPara->nFiberLenMax = atof(argv[i+1]);	//maximum fiber length
