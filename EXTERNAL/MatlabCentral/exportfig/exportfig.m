...
pltLines  = findobj(H, 'type', 'line' , '-and', 'Tag', 'plt_lne');
vrtLines  = findobj(H, 'type', 'line' , '-and', 'Tag', 'vrt_lne');
...
try
    ...
    if opts.separatetext & ~opts.applystyle
        ...
        oldtvis = LocalGetAsCell(allText,'visible');    if ~iscell(oldtvis{1}); oldtvis = {oldtvis}; end
        oldax = LocalGetAsCell(allAxes,'XTickLabel',1); if ~iscell(oldax{1});   oldax = {oldax}; end
        olday = LocalGetAsCell(allAxes,'YTickLabel',1); if ~iscell(olday{1});   olday = {olday}; end
        oldaz = LocalGetAsCell(allAxes,'ZTickLabel',1); if ~iscell(oldaz{1});   oldaz = {oldaz}; end
        ...
        if ~isempty(pltLines)
            allNonText = [pltLines; allLights; allPatch; ...
        ...
        while ischar(line)
            ...
            elseif (length(line) < 3)
                fprintf(fid1,'%s\n',line);
            elseif isequal(regexp(line,' *0 +0 +\d+ +\d+ +r[fe]', 'start'), 1)
                % Set the line to whitespace
        ...
        % Hide all text and export %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        args(end) = [];       
        
        for k=1:length(args)
            if strncmp('-d',args{k},2)
                args{k} = '-depsc'; end; end
        
        set(allText,'Color',[1 1 1]);
        set(allAxes,{'XColor'},repmat({[1 1 1]},numel(null),1));
        set(allAxes,{'YColor'},repmat({[1 1 1]},numel(null),1));
        set(allAxes,{'ZColor'},repmat({[1 1 1]},numel(null),1));
        oldvis = LocalGetAsCell(vrtLines,'visible');
        set(vrtLines,'Visible','off');
        
        print(H, filename, args{:});
        
        set(vrtLines,{'Visible'},oldvis);
        set(allText,'Color',[0 0 0]);
        set(allAxes,{'XColor'},repmat({[0 0 0]},numel(null),1));
        set(allAxes,{'YColor'},repmat({[0 0 0]},numel(null),1));
        set(allAxes,{'ZColor'},repmat({[0 0 0]},numel(null),1));
