...
%% Set up colors
% if ~isfield(pcfg,'aparc') || isempty(pcfg.aparc) % move to ejk_brain_plot Brain is light grey if no parcellation %%%%%%%%%%%%%%%%%%%%%%%%%
...
%     if isfield(pcfg,'inc_reg'); [ ~ , lbl_ind ] = intersect(mmil_switch_names(albl,pcfg.clr_fld),pcfg.inc_reg); else lbl_ind = 1:numel(albl); end
%     if size(albl,1) == size(col_loc,1)
%         for iCL = 1:size(col_loc,1);
%             if ismember(col_loc(iCL),lbl_ind)
%                 if albl(iCL);    col(iCL,:) = (actbl.table(albl(iCL)==actbl.table(:,5),1:3))/255; end;
%             else
%                 col(iCL,:) = [.7 .7 .7];
%             end
%         end
...
%         for iCL = 1:size(col_loc,1); 
%             if ismember(col_loc(iCL),lbl_ind)
%                 if col_loc(iCL); col(iCL,:) = (actbl.table(col_loc(iCL),1:3))/255; end;
%             else
%                 col(iCL,:) = [.7 .7 .7];
%             end
%         end
...
% else  % In use if  % In use if coloring in ROIs, move to ejk_roi_plot & ejk_roi_display_plot %%%%%%%%%%%%%%%%%%%%%%%%
%     
%     [col_loc,albl,actbl]=fs_read_annotation(pcfg.aparc);
%     if isfield(pcfg,'inc_reg'); [ ~ , lbl_ind ] = intersect(ejk_switch_names(albl,pcfg.clr_fld,'split'),pcfg.inc_reg); else lbl_ind = 1:numel(albl); end
%     
%     for iLC = 1:numel(albl)
%         tbl_ind = string_find(cellfun(@(x) x(4:end),pcfg.tbl_loc,'uni',0),albl{iLC});
%         if isfield(pcfg,'inc_reg'); if ~any(tbl_ind==lbl_ind); tbl_ind = []; end; end
...
%             end
%         else
%             col_hld(iLC,:) = rgb('light grey');%col_hld(iLC,:) = rgb('dark grey');
%         end
%     end
%     
%     col = repmat(rgb('white'), [size(sub.vert, 1) 1]);
...
trisurf(sub.tri, sub.vert(:, 1), sub.vert(:, 2), sub.vert(:, 3),...
    'FaceVertexCData', pcfg.col_map,'FaceColor', 'interp','FaceAlpha',pcfg.alpha,'parent',pcfg.axe_hnd);

shading(pcfg.axe_hnd,'interp');
lighting(pcfg.axe_hnd,'gouraud');
material(pcfg.axe_hnd,'dull');
axis(pcfg.axe_hnd,'off')
% axes(pcfg.axe_hnd); %light; 
if (isfield(pcfg,'sve_img') && ~isempty(pcfg.sve_img)) || (isfield(pcfg,'plv') && pcfg.plv); set(pcfg.fig_hdl,'Visible','off'); end
...
if isfield(pcfg,'rsp_ele') && isfield(pcfg,'color')
    ...
    elseif iscell(pcfg.color)
       ...
       cose all colormap(cell2mat(cat(1,pcfg.color,pcfg.nsl_col(1)))); freezeColors
    end
...
%         dbl_skp = zeros(1,numel(pcfg.rsp_ele{iGP}));
	...
	for i=1:size(pcfg.rsp_ele{iGP},1)
            ...
            if dbl_skp{iGP}(i)==1
                tmp_rsp = pcfg.rsp_ele;
                tmp_rsp(cellfun(@isempty,tmp_rsp)) = {[nan]};
                tmp_col = ~cellfun(@isempty,cellfun(@(x) intersect(pcfg.rsp_ele{iGP}(i,1),x),tmp_rsp,'uni',0));
