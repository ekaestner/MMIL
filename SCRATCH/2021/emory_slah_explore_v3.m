...
srf_dir_hld( string_find(srf_dir_hld,'__')) = [];
...
col_nme = { 'SubjID' 'Sequence' 'For_Outcome' 'Rev_Outcome' ...
    'For_Name' 'For_Dir' 'For_Num' 'For_B0' 'For_B1000' 'For_B2000' ...
    'Rev_Name' 'For_Dir' 'Rev_Num' 'Rev_B0' 'Rev_B1000' 'Rev_B2000'};
out_tbl = cell( size(scn_cls,1), numel(col_nme));
for iS = 1:size(scn_cls,1)
    ...
    if numel(dti_scn_col)==2
        ...
        [ ~, dcm_hed] = unix(['dcmdump ' dta_dir '/' sbj_dir_use{1} '/' scn_dir{scn_dir_for} '/' tmp_dir_hld{1} '| grep -i ''0018,1312'' | sort']);
        dcm_hed_beg = strfind(dcm_hed,'CS [');
        dcm_hed_end = regexp(dcm_hed,']');        
        out_tbl{iS,6} = dcm_hed(dcm_hed_beg+4:dcm_hed_end-1);
        
        out_tbl{iS,7} = numel(tmp_dir_hld);
        ...
        out_tbl{iS,8} = sum(bvl_hld<50);
        out_tbl{iS,9} = sum(bvl_hld>950 & bvl_hld<1050);
        out_tbl{iS,10} = sum(bvl_hld>1950 & bvl_hld<2050);
        %%%%%%%
        ...
        out_tbl{iS,11} = dcm_inf.ProtocolName;
        
        [ ~, dcm_hed] = unix(['dcmdump ' dta_dir '/' sbj_dir_use{1} '/' scn_dir{scn_dir_rev} '/' tmp_dir_hld{1} ' | grep -i ''0018,1312'' | sort']);
        dcm_hed_beg = strfind(dcm_hed,'CS [');
        dcm_hed_end = regexp(dcm_hed,']');        
        out_tbl{iS,12} = dcm_hed(dcm_hed_beg+4:dcm_hed_end-1);
  
        
        out_tbl{iS,13} = numel(tmp_dir_hld);
        ...
        out_tbl{iS,14} = sum(bvl_hld<50);
        out_tbl{iS,15} = sum(bvl_hld>950 & bvl_hld<1050);
        out_tbl{iS,16} = sum(bvl_hld>1950 & bvl_hld<2050);
        %%%%%%%
