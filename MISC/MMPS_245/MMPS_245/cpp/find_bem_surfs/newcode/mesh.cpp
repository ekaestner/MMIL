...
mesh::mesh(const string &mriInputFileName, 
	   int talairachFlag, 
	   string talairachFileName, 
	   string triInputDirectory): 
  mri(mriInputFileName) {

  const int maxMeshNumber=8; 

  cout << "Mesh number" << endl; cin >> meshNumber; 

  if(meshNumber > maxMeshNumber) {
    cout << "Wrong mesh number in mesh::mesh" << endl; 
    abort(); 
  }
  ...
  this->triInputDirectory=triInputDirectory; 
  ...
  ifstream triFile(sDummy.c_str());

  if(!triFile.is_open()) {
    cout << "Unable to open tri-file in function mesh::mesh" << endl; 
    abort(); 
  }
...
/* compute solid angle based on oosteron and Strackee 
   [IEEE Trans. BME, 30, 125-126, 1983] */  

/* WARNING - using (0.,0.,0.) as the origin */ 

float mesh::computeSolidAngle() {
  ...
  double r[3]; 
  float **vr; 
  double r0r1, r0r2, r1r2, dummy; 

  vr = new float* [3]; 
  for (j=0; j < 3; j++) vr[j] = new float [3]; 
  ...
  for(k=0; k < nFace; k++) {
    ...
    for(j=0; j < 3; j++) { 
      ...
      for(i=0; i < 3; i++) {
	vr[j][i] = vertex[face[k].nTri[j]].r[i]; 
	dummy += vr[j][i]*vr[j][i]; 
      }
    ...
    dummy = r[0]*r[1]*r[2]+r0r1*r[2]+r0r2*r[1]+r1r2*r[0]; 

    angle += atan(computeDeterminant(vr,3)/dummy); 

  }

  /* WARNING - use a negative value for the angle */
...
/* compute average intensity inside the filled volume */ 

float mesh::computeInsideIntensity() {

  int jByte, jRow, jSlice; 
  float number=0., dummy=0., averageIntensity; 
  ...
  for (jByte = 1; jByte < (nByte-1); jByte++)
	...
	if(contourFilling[jByte][jRow][jSlice]==weightInside) {
	  dummy += dataMghVolume[jByte][jRow][jSlice]; 
          number += 1.; 
	}
  
  averageIntensity=dummy/number;
...
void mesh::importSurface(string surfaceName, 
  ...
  if(!surfaceInputFile.is_open()) {
    cout << "Unable to open surface input file " << sDummy << " in function "  
