...
/* same as triFilePlotter, but now includes Talairach transform and 
   use VETSA subject number as input */ 
...
string surfacePath="/export/space/monkeys/3/igor/VETSA/DETERMINISTIC/TRI/"; 
string talairachPath="/export/space/monkeys/3/igor/VETSA/XFM/"; 
string subjectName;  
int nView;
int nSurface;  

float factor=85.; 
float shift[3] = {0., 0.4, 0.}; 
...
void display(void)
{
  ...
  float tempInput[4], tempOutput[4];

  for (k=0; k < nVertex; k++) {

    for(i=0; i < 3; i++) tempInput[i]=vdata[k][i];
    tempInput[3]=1.; 

    for(j=0; j < 4; j++) {
      tempOutput[j]=0.; 
      for(i=0; i < 4; i++) 
	tempOutput[j] += talairach[j][i]*tempInput[i]; 
    }	

    for(i=0; i < 3; i++) {
      vdata[k][i] = tempOutput[i]; 
    }
  }
  ...
  if( (nView==1) || (nView==2) ) { 
    ...
    for (j=0; j < nVertex; j++) 
      for(i=0; i < 3; i++) vdata[j][i] += shift[i]; 
