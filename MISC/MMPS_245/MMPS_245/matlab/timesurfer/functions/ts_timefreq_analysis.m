function ts_timefreq_analysis (varargin)

% Use ts_timefreq_analysis ('epoch_dir',epoch_dir...);
% Note: this function replaced ts_iEEG_FreqAnalysis
%
...
if isempty(opt.toi),opt.toi = epoch_data.epochs(1).time; end;
...
str_foi = sprintf('foi%d-%d',opt.foi(1),opt.foi(end));
str_toi = sprintf('toi%.3g-%.3g',opt.toi(1)/1000,opt.toi(end)/1000);
...
if opt.makebaseline_flag
    ...
    if opt.savesingles_flag
        try baseline_data.timefreq.power = single(baseline_data.timefreq.power); end
        try baseline_data.timefreq.cmplx = single(baseline_data.timefreq.cmplx); end        
        try baseline_data.timefreq.data = single(baseline_data.timefreq.data); end
    end
    if ~opt.savecomplex_flag
        try baseline_data.timefreq = rmfield(baseline_data.timefreq,'cmplx'); end
    end
...
for i = 1:length(timefreq_conditions)
    ...
    if opt.savesingles_flag
        try timefreq_data.timefreq.power = single(timefreq_data.timefreq.power); end
        try timefreq_data.timefreq.cmplx = single(timefreq_data.timefreq.cmplx); end        
        try timefreq_data.timefreq.data = single(timefreq_data.timefreq.data); end
    end    
    if ~opt.savecomplex_flag
        try timefreq_data.timefreq = rmfield(timefreq_data.timefreq,'cmplx'); end
    end    
    % if all data in "data" are nan, remove the "data" field
    if all(all(all(isnan(timefreq_data.timefreq.data)))), timefreq_data.timefreq = rmfield(timefreq_data.timefreq,'data'); end;
    if strfind(file_tag,'event')
