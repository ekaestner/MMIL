...
%   'overflow_detect' 1 or 0, default = []  %%%%% SRD 083012
%
...
%
% Last Mod SRD 082912 Added option overflow_detect to force off overflow detect for ts_loadedf 
...
elseif nargin==8 && isstruct(varargin{1}) &&  strcmp(varargin{6},'overflow_detect')  %%%%% SRD 083012: overflow detection option for the sake of ts_loadedf
  % input appears to be filename, hdr, begsample, endsample, chanindx, continuous, 'overflow_detect', overflow_detect
  continuous    = varargin{5};
  checkboundary = (continuous==0);  % the default is to check in case the file is _not_ continuous
  overflow_detect = varargin{7};
  varargin = {'header', varargin{1}, 'begsample', varargin{2}, 'endsample', varargin{3}, 'chanindx', varargin{4}, ...
              'checkboundary', checkboundary, 'overflow_detect', overflow_detect}; %%%%% SRD 083012
end
...
overflow_detect=keyval('overflow_detect', varargin); %%%%% SRD 083012
if overflow_detect==[] overflow_detect=1; end %%%%% SRD 083012
...
switch dataformat
  ...
  case {'biosig', 'edf'}
    ...
    hastoolbox('BIOSIG', 1);
    dat = read_biosig_data(filename, hdr, begsample, endsample, chanindx, 'overflow_detect', overflow_detect);  %%%%% SRD 083012
