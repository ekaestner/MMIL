function hdr = load_nifti_hdr(fname)
% hdr = load_nifti_hdr(fname)
%
...
% When opening again, use fp = fopen(fname,'r',hdr.endian);
%
...
% Last Mod:               08/15/2017 by Feng Xue
%
...
if(nargin ~= 1)
  fprintf('hdr = load_nifti_hdr(fname)\n');
  return;
end
...
if(hdr.sizeof_hdr ~= 348)
  ...
  if(hdr.sizeof_hdr ~= 348)
    fclose(fp);
    error('ERROR: %s: hdr size = %d, should be 348\n',...
	    fname,hdr.sizeof_hdr);
  end
  hdr.endian = 'l';
else
  hdr.endian = 'b';
