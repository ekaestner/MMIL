function resamp_results = MEG_MMIL_dSROI_GroupAvg_Resamp(fnames,varargin)
%function resamp_results = MEG_MMIL_dSROI_GroupAvg_Resamp(fnames,[options])
%
% Purpose: Calculate mean and variation across subjects
%   of a set of dSPM-ROI waveforms using bootstrap resampling
%
% Required Input:
...
%
% Created:  04/24/12 by Don Hagler
...
function parms = check_input(fnames,options)
  parms_filter = {...
    'fnames',fnames,[],...
    'outdir',[pwd '/dSROI_GroupAvg_Resamp'],[],...
    'outstem','dSROI',[],...
...
function resamp_results = calc_cond_avg(parms,resamp_results)
  cond_name = 'average';
  for r=1:parms.nrois
    roi_name = resamp_results(r,1).roi;
    if parms.peak_flag
      ...
      resamp_results(r,parms.nconds+1).peak = peak;
      resamp_results(r,parms.nconds+1).roi = roi_name;
      resamp_results(r,parms.nconds+1).cond = cond_name;
    end;
    ...
    if parms.onset_flag
      ...
      for c=1:parms.nconds
        if isempty(onset)
          onset = resamp_results(r,c).onset;
        else
          onset.latencies = onset.latencies + ...
            resamp_results(r,c).onset.latencies;
          if parms.bias_corr_flag
            onset.latencies_jackknife = onset.latencies_jackknife + ...
              resamp_results(r,c).onset.latencies_jackknife;
          end;
          onset.latency_mean = onset.latency_mean + ...
            resamp_results(r,c).onset.latency_mean;
        end;
      end;
      ...
      % calculate bootstrap sem
      onset.latency = ts_nan_std(onset.latencies,0,2);
      % calculate confidence intervals
      if parms.bias_corr_flag
      ...
      % add to resamp_results
      resamp_results(r,parms.nconds+1).onset = onset;
      resamp_results(r,parms.nconds+1).roi = roi_name;
      resamp_results(r,parms.nconds+1).cond = cond_name;
    end;
    ...
    if parms.auc_flag
      ...
      for c=1:parms.nconds
        if isempty(auc)
          auc = resamp_results(r,c).auc;
        else
          auc.vals = auc.vals + ...
            resamp_results(r,c).auc.vals;
          if parms.bias_corr_flag
            auc.vals_jackknife = auc.vals_jackknife + ...
              resamp_results(r,c).auc.vals_jackknife;
          end;
          auc.val_mean = auc.val_mean + ...
            resamp_results(r,c).auc.val_mean;
        end;
      end;
      ...
      % add to resamp_results
      resamp_results(r,parms.nconds+1).auc = auc;
      resamp_results(r,parms.nconds+1).roi = roi_name;
      resamp_results(r,parms.nconds+1).cond = cond_name;
    end;
  end;
