function resamp_results = MEG_MMIL_Sensor_GroupAvg_Diff_Resamp(fname_sets,varargin)
%function resamp_results = MEG_MMIL_Sensor_GroupAvg_Diff_Resamp(fname_sets,[options])
%
% Purpose: Compare two sets of sensor waveforms across subjects
%   using bootstrap resampling
%
% Required Input:
%   fname_sets: cell array with two elements, each containing cell array
%     of Sensor_GroupAvg results mat files to be averaged together
%
% Optional Input ('key',arg pairs):
%  'outdir': output directory
%    {default = [pwd '/Sensor_GroupAvg_Diff_Resamp']}
...
function parms = check_input(fname_sets,options)
  parms_filter = {...
    ...
    'outdir',[pwd '/Sensor_GroupAvg_Diff_Resamp'],[],...
    'outstem','Sensor_Diff',[],...
...
function resamp_results = calc_cond_avg(parms,resamp_results)
  ...
  if parms.peak_flag
    ...
    for i=1:length(tags)
      ...
      resamp_results(parms.nconds+1).(tag) = peak;
  ...
  if parms.onset_flag
    ...
    for i=1:length(tags)
      ...
      % add to resamp_results
      resamp_results(parms.nconds+1).(tag) = onset;
      resamp_results(parms.nconds+1).cond = cond_name;
    end;
  end;
  ...
  if parms.auc_flag
    ...
    for i=1:length(tags)
      ...
      for c=1:parms.nconds
        if isempty(auc)
          auc = resamp_results(c).(tag);
        else
          auc.vals = auc.vals + ...
            resamp_results(c).(tag).vals;
          if parms.bias_corr_flag
            auc.vals_jackknife = auc.vals_jackknife + ...
              resamp_results(c).(tag).vals_jackknife;
          end;
          auc.val_mean = auc.val_mean + ...
            resamp_results(c).(tag).val_mean;
        end;
      end;
      ...
      % add to resamp_results
      resamp_results(parms.nconds+1).(tag) = auc;
      resamp_results(parms.nconds+1).cond = cond_name;
    end;
  end;
return;
...
function parms = plot_wforms_diff(parms,resamp_results)
  ...
  plot_parms.outstem = sprintf('%s_wforms_diff',parms.outstem);
  if parms.title_flag
    plot_parms.title = ...
...
function parms = plot_wforms_pair(parms,resamp_results)
  ...
  for c=1:parms.nconds
    ...
    % set plotting parameters
    plot_parms = parms.plot_parms;
    plot_parms.outstem = sprintf('%s_%s_wforms_pair',...
      parms.outstem,cond);
    if parms.title_flag
      plot_parms.title = ...
        sprintf('%s group %s response',...
          regexprep(parms.outstem,'_',' '),cond);
    end;
    if parms.legend_flag
...
function parms = plot_wforms_conds(parms,resamp_results)
  ...
  for s=1:2
    ...
    plot_parms = parms.plot_parms;
    plot_parms.outstem = ...
      sprintf('%s_%s_wforms',parms.outstem,outfix);
    if parms.title_flag
      plot_parms.title = ...
        sprintf('%s group %s response',...
          regexprep(parms.outstem,'_',' '),outfix);
    end;
    if parms.legend_flag
      plot_parms.condnames = parms.condnames;
    end;
...
function parms = plot_latency_diff(parms,resamp_results,type_str)
  ...
  for c=1:parms.nconds
    ...
    rplot_parms.responses_err(1,c,:) = ...
      resamp_results(1,c).(type_str).latency_ci;
  end;
  figure(parms.f); parms.f = parms.f + 1;
...
function parms = plot_latency_pair(parms,resamp_results,type_str)
  fprintf('%s: plotting %s latency for %s...\n',...
    mfilename,type_str,parms.outstem);    
  rplot_parms = parms.rplot_parms;
  rplot_parms.outstem = sprintf('%s_%s_latency',...
    parms.outstem,type_str);
  if parms.title_flag
    rplot_parms.title = ...
...
function parms = plot_peak_amplitude_pair(parms,resamp_results,normflag)
  ...
  rplot_parms = parms.rplot_parms;
  if normflag
    rplot_parms.outstem = sprintf('%s_peak_amplitude_norm',...
      parms.outstem);
    if parms.title_flag
      rplot_parms.title = ...
    ...
    rplot_parms.outstem = sprintf('%s_peak_amplitude',...
      parms.outstem);
    if parms.title_flag
  ...
  responses = zeros(2,parms.nconds);
  for c=1:parms.nconds
    responses(1,c) = resamp_results(c).peak1.amplitude;
    rplot_parms.responses_err(1,c,:) = resamp_results(c).peak1.amplitude_ci;
    responses(2,c) = resamp_results(c).peak2.amplitude;
    rplot_parms.responses_err(2,c,:) = resamp_results(c).peak2.amplitude_ci;
  end;
  figure(parms.f); parms.f = parms.f + 1;
  rplot_args = mmil_parms2args(rplot_parms);
  ts_plot_responses(responses,rplot_args{:});
...
function parms = plot_area_under_curve_diff(parms,resamp_results,normflag)
  ...
  for c=1:parms.nconds
    ...
    rplot_parms.responses_err(1,c,:) = resamp_results(c).auc.ci;
  end;
  figure(parms.f); parms.f = parms.f + 1;
  rplot_args = mmil_parms2args(rplot_parms);
  ts_plot_responses(responses,rplot_args{:});
...
function parms = plot_area_under_curve_pair(parms,resamp_results,normflag)
  ...
  responses = zeros(2,parms.nconds);
  for c=1:parms.nconds
    responses(1,c) = resamp_results(c).auc1.mean;
    rplot_parms.responses_err(1,c,:) = resamp_results(c).auc1.ci;
    responses(2,c) = resamp_results(c).auc2.mean;
    rplot_parms.responses_err(2,c,:) = resamp_results(c).auc2.ci;
  end;
  figure(parms.f); parms.f = parms.f + 1;
  rplot_args = mmil_parms2args(rplot_parms);
  ts_plot_responses(responses,rplot_args{:});
