function avg_results = MEG_MMIL_dSROI_GroupAvg_CondAvg(fnames,varargin)
%function avg_results = MEG_MMIL_dSROI_GroupAvg_CondAvg(fnames,[options])
%
% Purpose: Calculate mean and variation across subjects
%   of a set of dSPM-ROI waveforms
%
% Required Input:
%   fnames: cell array of dSROI_GroupAvg results mat files
%     to be averaged together
%
% Optional Input ('key',arg pairs):
%  'outdir': output directory
%    {default = [pwd '/dSROI_GroupAvg_Resamp']}
...
%% todo: add more options to help (e.g. fnames_hemi)
...
if ~exist(fname_results,'file') || parms.forceflag
  ...
  % average waveforms
  fprintf('%s: averaging waveforms for %s...\n',mfilename,parms.outstem);
  avg_results = avg_wforms(parms,results);
  % calculate stats for across-condition average
  if parms.cond_avg_flag
    fprintf('%s: averaging across conditions for %s...\n',...
      mfilename,parms.outstem);
    avg_results = calc_cond_avg(parms,avg_results);
  end;
...
%      mfilename,parms.outstem);
%    summarize_results(parms,avg_results);
  end;
  % save results to mat file
  save(fname_results,'avg_results','-v7.3');
else
...
if parms.plot_wforms_flag
  ...
  parms = plot_wforms(parms,avg_results);
end;
% plot responses
%% todo: plot responses
%if parms.plot_resps_flag
%  fprintf('%s: plotting responses for %s...\n',mfilename,parms.outstem);
%  parms = plot_responses(parms,avg_results);
%end;
...
function parms = check_input(fnames,options)
  parms_filter = {...
    'fnames',fnames,[],...
    'outdir',[pwd '/dSROI_GroupAvg_CondAvg'],[],...
    'outstem','dSROI',[],...
    ...
    'plot_resps_flag',false,[false,true],...
    ...
    'onset_flag',false,[false true],... %% todo: analyze waveforms
  ...
  if ~isempty(parms.fnames_hemi) && length(parms.fnames_hemi)~=parms.nfiles
    error('fnames_hemi must match fnames');
  end;
  parms.f = 1;
return;
...
%% todo: parameters for analyzing waveforms
if 0
...
function avg_results = avg_wforms(parms,results)
  clear avg_results;
%  resamp_args = mmil_parms2args(parms.resamp_parms);
  for r=1:parms.nrois
    ...
    for f=1:parms.nfiles
      ...
      if ~isempty(parms.fnames_hemi)
        hemilist = parms.hemilist(parms.fnames_hemi(f));
      else
    ...
    wforms = wforms / (parms.nfiles * length(hemilist));
    ...
    for c=1:parms.nconds
      ...
      tmp_results = [];
      if parms.nsubs>1
        tmp_results.wforms_mean = mean(tmp_wforms,2);
      else
        tmp_results.wforms_mean = tmp_wforms;
      end;
      if parms.nsubs==1
        tmp_results.wforms_stderr = zeros(size(tmp_wforms));
      elseif parms.nsubs==2
        tmp_results.wforms_stderr = std(tmp_wforms,1,2);
      else
        tmp_results.wforms_stderr = std(tmp_wforms,0,2)/sqrt(parms.nsubs);
      end;
      %% todo: use rc_analyze_wforms to get peaks, etc.
      tmp_results.roi = parms.roinames{r};
      tmp_results.cond = parms.condnames{c};
      avg_results(r,c) = tmp_results;
    end;
  end;
return;
...
function avg_results = calc_cond_avg(parms,avg_results)
  cond_name = 'average';
  for r=1:parms.nrois
    roi_name = avg_results(r,1).roi;
    if parms.peak_flag
      ...
      % add to avg_results
      avg_results(r,parms.nconds+1).peak = peak;
      avg_results(r,parms.nconds+1).roi = roi_name;
      avg_results(r,parms.nconds+1).cond = cond_name;
    end;
    ...
    if parms.onset_flag
      ...
      for c=1:parms.nconds
        if isempty(onset)
          onset = avg_results(r,c).onset;
        else
          onset.latencies = onset.latencies + ...
            avg_results(r,c).onset.latencies;
          if parms.bias_corr_flag
            onset.latencies_jackknife = onset.latencies_jackknife + ...
              avg_results(r,c).onset.latencies_jackknife;
          end;
          onset.latency_mean = onset.latency_mean + ...
            avg_results(r,c).onset.latency_mean;
        end;
      end;
      ...
      % add to avg_results
      avg_results(r,parms.nconds+1).onset = onset;
      avg_results(r,parms.nconds+1).roi = roi_name;
      avg_results(r,parms.nconds+1).cond = cond_name;
    end;
    ...
    if parms.auc_flag
      ...
      for c=1:parms.nconds
        if isempty(auc)
          auc = avg_results(r,c).auc;
        else
          auc.vals = auc.vals + ...
            avg_results(r,c).auc.vals;
          if parms.bias_corr_flag
            auc.vals_jackknife = auc.vals_jackknife + ...
              avg_results(r,c).auc.vals_jackknife;
          end;
          auc.val_mean = auc.val_mean + ...
            avg_results(r,c).auc.val_mean;
        end;
      end;
      ...
      % add to avg_results
      avg_results(r,parms.nconds+1).auc = auc;
      avg_results(r,parms.nconds+1).roi = roi_name;
      avg_results(r,parms.nconds+1).cond = cond_name;
    end;
  end;
...
function parms = plot_wforms(parms,avg_results)
  ...
  for r=1:parms.nrois
    ...
    plot_parms.outstem = sprintf('%s_%s_wforms',parms.outstem,roiname);
    if parms.title_flag
    ...
    plot_parms.ranges = [];
    for c=1:parms.nconds
      tmp_results = avg_results(r,c);
      wforms(:,c) = tmp_results.wforms_mean;
      if parms.wforms_err_flag
        plot_parms.wforms_err(:,c,1) = ...
          tmp_results.wforms_mean - tmp_results.wforms_stderr;
        plot_parms.wforms_err(:,c,2) = ...
          tmp_results.wforms_mean + tmp_results.wforms_stderr;
      end;
...
function parms = plot_responses(parms,avg_results)
  % plot peak latency vs. condition value
  if parms.peak_flag
    parms = plot_latency(parms,avg_results,'peak');
  end;
  % plot onset latency vs. condition value
  if parms.resamp_parms.onset_flag
    parms = plot_latency(parms,avg_results,'onset');
  end;
  % plot peak amplitude vs. condition value
  if parms.peak_flag
    for normflag=0:1
      parms = plot_peak_amplitude(parms,avg_results,normflag);
    end;
  end;
  % plot area under the curve vs. condition value
  if parms.resamp_parms.auc_flag
    for normflag=0:1
      parms = plot_area_under_curve(parms,avg_results,normflag);
    end;
  end;
return;
...
function parms = plot_latency(parms,avg_results,type_str)
  ...
  for r=1:parms.nrois
    for c=1:parms.nconds
      responses(r,c) = avg_results(r,c).(type_str).latency;
      rplot_parms.responses_err(r,c,:) = avg_results(r,c).(type_str).latency_ci;
    end;
  end;
  figure(parms.f); parms.f = parms.f + 1;
...
function parms = plot_peak_amplitude(parms,avg_results,normflag)
  rplot_parms = parms.rplot_parms;
  if normflag
  ...
  for r=1:parms.nrois
    for c=1:parms.nconds
      responses(r,c) = avg_results(r,c).peak.amplitude;
      rplot_parms.responses_err(r,c,:) = avg_results(r,c).peak.amplitude_ci;
    end;
  end;
  figure(parms.f); parms.f = parms.f + 1;
  rplot_args = mmil_parms2args(rplot_parms);
...
function parms = plot_area_under_curve(parms,avg_results,normflag)
  rplot_parms = parms.rplot_parms;
  if normflag
  ...
  for r=1:parms.nrois
    for c=1:parms.nconds
      responses(r,c) = avg_results(r,c).auc.mean;
      rplot_parms.responses_err(r,c,:) = avg_results(r,c).auc.ci;
    end;
  end;
  figure(parms.f); parms.f = parms.f + 1;
  rplot_args = mmil_parms2args(rplot_parms);
...
function summarize_results(parms,avg_results)
  outstem = [parms.outdir '/' parms.outstem];
%% todo: function to summarize avg_results
%  ts_summarize_resamp_results(avg_results,'fstem_out',outstem);
return;
