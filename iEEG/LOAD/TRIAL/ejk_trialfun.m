...
function [trl, evt] = ejk_trialfun(fcfg)
...
for iD = 1:numel(fcfg.dataset)
    ...
    if any(strfind(fcfg.trialfun_add{iD},'---'))
        ...
        for iTF = 1:numel(trialfun_add_hld)
            ...
            if strcmpi(trialfun_add_hld{iTF}(end-3:end),'.csv')
                ...
                evt_hld{iTF}(1,:) = [];
            elseif strcmpi(trialfun_add_hld{iTF}(end-3:end),'.log')
                evt_hld{iTF} = mmil_readtext([fcfg.prj_dat_hld '/' 'clerical' '/' 'task' '/' fcfg.sbj_nme '/' trialfun_add_hld{iTF}],[',']);
                if any(strcmpi(evt_hld{iTF}(:,3),'Response')); evt_hld{iTF}(strcmpi(evt_hld{iTF}(:,3),'Response'),:) = []; end
                if any(cell2mat(evt_hld{iTF}(:,4))==0); evt_hld{iTF}(cell2mat(evt_hld{iTF}(:,4))==0,:) = []; end
            end
        end
    ...
    % Check data
    cell2csv([fcfg.prj_dat_hld '/' 'clerical' '/' 'task' '/' fcfg.sbj_nme '/' 'TimeTest' '_' fcfg.trialfun_add{iD}],num2cell([diff(samp)' diff(cell2mat(evt_hld(:,2)) / (10 * (1000/Fs)) ) floor(diff(samp)'-diff(cell2mat(evt_hld(:,2)) / (10 * (1000/Fs))))  ]));
