function dat = mmil_update_cmn_nse(cfg,dat)

%% Update cmn_nse
run_typ = mmil_load_subj_info([cfg.clr_fld '/' 'sbj_inf' '/' cfg.sbj_nme],'cmn_nse');

if cfg.sub_fld_ind <= numel(run_typ) && all(~strcmpi(run_typ,'split') & ~isfield(dat.cfg.rmv_nse,'spl_chn'))
    ...
    cur_cmn_nse = dat.cfg.rmv_nse.spl_chn;  
    cur_cmn_nse_nme = dat.cfg.rmv_nse.spl_nme;
    
    rmv_ind = cellfun(@isempty,cur_cmn_nse);
    cur_cmn_nse(rmv_ind) = [];
    cur_cmn_nse_nme(rmv_ind) = [];
    dat.cfg.rmv_nse.pre_fix(rmv_ind) = [];
end

cur_cmn_nse = cur_cmn_nse;
hld_org_nse = cur_cmn_nse;
cur_cmn_nse_nme = cur_cmn_nse_nme;

new_cmn_nse_rmv = cell(1);
new_cmn_nse = cell(1);
for iCN = 1:numel(cur_cmn_nse)   
      
    load([cfg.out_dir '/' cfg.sbj_nme '/' dat.cfg.rmv_nse.pre_fix{iCN} '/' 'rmv_num.mat'])
...
for iN = 1:numel(new_cmn_nse)
    if ~isempty(new_cmn_nse{iN})
        new_nme_hld = [new_nme_hld cur_cmn_nse_nme{iN}];
        new_nme_hld = [new_nme_hld ','];
    end
end
new_nme_hld(end) = '}';

cur_str_pst = mmil_load_subj_info([cfg.clr_fld '/' 'sbj_inf' '/' cfg.sbj_nme],'cmn_nme','asstring');
if ~strcmpi(run_typ(1),'split') || ~strcmpi(cur_str_pst{1},'[]')
    ...
    cur_cmn_nme = mmil_load_subj_info([cfg.clr_fld '/' 'sbj_inf' '/' cfg.sbj_nme],'cmn_nme','asstring');
    cur_cmn_nme = strsplit(cur_cmn_nme{1},';');
    cur_cmn_nme{cfg.sub_fld_ind} = new_nme_hld;
    cur_cmn_nme = [strjoin(cur_cmn_nme,';') ';'];
    mmil_load_subj_info([cfg.clr_fld '/' 'sbj_inf' '/' cfg.sbj_nme],'cmn_nme',cur_cmn_nme);
else
...
for iP = 1:numel(hld_org_nse)
    ...
    if ~isempty(oth_chn{iP})
        ...
        % 1st Plot
        if ~isdir([cfg.out_dir '/'  cfg.sbj_nme '/' dat.cfg.rmv_nse.pre_fix{iP} '/' 'rmv_chn']); mkdir([cfg.out_dir '/'  cfg.sbj_nme '/' dat.cfg.rmv_nse.pre_fix{iP} '/' 'rmv_chn']); end
        ...
        for iRN = 1:numel(cfg.rnd_ind)
            ...
            plot(dat.time{1},ttt_men,'k','LineWidth',1.25);
            xlim([cfg.rnd_ind(iRN) cfg.rnd_ind(iRN)+2]);
            print(gcf,[cfg.out_dir '/' cfg.sbj_nme '/' dat.cfg.rmv_nse.pre_fix{iP} '/' 'rmv_chn' '/' dat.cfg.rmv_nse.pre_fix{iP} '_' num2str(iRN) '.png'],'-dpng');
        end
        
        % Save #'s
        tmp_chn_str = num_2_str(oth_chn{iP});
        cell2csv([cfg.out_dir '/' cfg.sbj_nme '/' dat.cfg.rmv_nse.pre_fix{iP} '/' 'rmv_chn' '/' 'chn_str.csv'],{tmp_chn_str})

    end
end

dat.cfg.rmv_nse = rmfield(dat.cfg.rmv_nse,'spl_chn');
