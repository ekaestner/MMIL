function intan = read_Intan_RHD2000_file(fileName,pathName,varargin)
...
if (num_amplifier_channels > 0)
    intan.amplifier_channels = amplifier_channels;
    if (data_present)
        intan.amplifier_data = amplifier_data;
        intan.t_amplifier = t_amplifier;
    end
    intan.spike_triggers = spike_triggers;
end

if (num_aux_input_channels > 0)
    intan.num_aux_input_channels = num_aux_input_channels;
    if (data_present)
        intan.aux_input_data = aux_input_data;
        intan.t_aux_input = t_aux_input;
    end
end

if (num_supply_voltage_channels > 0)
    intan.num_supply_voltage_channels = num_supply_voltage_channels;
    if (data_present)
        intan.supply_voltage_data = supply_voltage_data;
        intan.t_supply_voltage = t_supply_voltage;
    end
end
...
if (num_board_dig_out_channels > 0)
    intan.board_dig_out_channels = board_dig_out_channels;
    if (data_present)
        intan.board_dig_out_data = board_dig_out_data;
        intan.t_dig = t_dig;
    end
end
...
if (num_temp_sensor_channels > 0)
    if (data_present)
        intan.temp_sensor_data = temp_sensor_data;
        intan.t_temp_sensor = t_temp_sensor;
    end
end
