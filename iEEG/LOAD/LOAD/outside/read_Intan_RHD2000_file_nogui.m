function out = read_Intan_RHD2000_file_nogui(fname)
...
% Modifed from read_Intan_RHD2000_file.m v2.01 from by Burke Rosen 2019/03/25 : 
%   Remove gui, assign output variable, and simplified standard output
%   ToDo: load specific ranges of channels or samples 
...
filename = file;
out = struct();
out = move_to_base_workspace(out,filename);
out = move_to_base_workspace(out,path);

out = move_to_base_workspace(out,notes);
out = move_to_base_workspace(out,frequency_parameters);
if (data_file_main_version_number > 1)
    out = move_to_base_workspace(out,reference_channel);
end

if (num_amplifier_channels > 0)
    out = move_to_base_workspace(out,amplifier_channels);
    if (data_present)
        out = move_to_base_workspace(out,amplifier_data);
        out = move_to_base_workspace(out,t_amplifier);
    end
    out = move_to_base_workspace(out,spike_triggers);
end
if (num_aux_input_channels > 0)
    out = move_to_base_workspace(out,aux_input_channels);
    if (data_present)
        out = move_to_base_workspace(out,aux_input_data);
        out = move_to_base_workspace(out,t_aux_input);
    end
end
if (num_supply_voltage_channels > 0)
    out = move_to_base_workspace(out,supply_voltage_channels);
    if (data_present)
        out = move_to_base_workspace(out,supply_voltage_data);
        out = move_to_base_workspace(out,t_supply_voltage);
    end
end
if (num_board_adc_channels > 0)
    out = move_to_base_workspace(out,board_adc_channels);
    if (data_present)
        out = move_to_base_workspace(out,board_adc_data);
        out = move_to_base_workspace(out,t_board_adc);
    end
end
if (num_board_dig_in_channels > 0)
    out = move_to_base_workspace(out,board_dig_in_channels);
    if (data_present)
        out = move_to_base_workspace(out,board_dig_in_data);
        out = move_to_base_workspace(out,t_dig);
    end
end
if (num_board_dig_out_channels > 0)
    out = move_to_base_workspace(out,board_dig_out_channels);
    if (data_present)
        out = move_to_base_workspace(out,board_dig_out_data);
        out = move_to_base_workspace(out,t_dig);
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        out = move_to_base_workspace(out,temp_sensor_data);
        out = move_to_base_workspace(out,t_temp_sensor);
    end
end
...
%%% BQR 2019/03/25
function out = move_to_base_workspace(out,variable)
%
% Move variable from function workspace to output structure
%
