...
%%
rho_tst_tbl = cell(numel([cor_nme grp_nme ]),3);
rvl_tst_tbl = cell(numel([cor_nme grp_nme ]),3);
...
for iCR = 1:numel(cor_nme)
    ...
    if strcmpi(rho_str(1),'-')
        rho_tst_tbl(tst_ind,:) = [ {rho} {poh} [ 'r=' rho_str([1 3:end]) '; p=' poh_str(2:end)] ];
    else
        rho_tst_tbl(tst_ind,:) = [ {rho} {poh} [ 'r=' rho_str(2:end) '; p=' poh_str(2:end)] ];
    end
    
    rvl_str = num2str(roundsd(rvl,2));
    pvl_str = num2str(roundsd(pvl,2));
    if strcmpi(rvl_str(1),'-')
        rvl_tst_tbl(tst_ind,:) = [ {rvl} {pvl} [ 'r=' rvl_str([1 3:end]) '; p=' pvl_str(2:end)] ];
    else
        rvl_tst_tbl(tst_ind,:) = [ {rvl} {pvl} [ 'r=' rvl_str(2:end) '; p=' pvl_str(2:end)] ];
    end
...
for iAN = 1:numel(grp_nme)
    ...
    rho_tst_tbl(tst_ind,:) = [ tst_hld(2,5) {pvl_hld} [ 'F(' num2str(stt_hld.df) ')=' fvl_str '; p=' pvl_str(2:end)]];
    rvl_tst_tbl(tst_ind,:) = [ tst_hld(2,5) {pvl_hld} [ 'F(' num2str(stt_hld.df) ')=' fvl_str '; p=' pvl_str(2:end)]];
