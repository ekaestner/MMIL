...
if ~exist(['/space/mdeh4/1/halgdev/projects/mmilanguage/SL/epoch_data/clerical' '/trialfun_output/' subj '.mat'])
    ...
    cfg.pos         = 3;
    cfg.ignore      = 1;
    if strcmpi(subj,'NY439_SL') || strcmpi(subj,'NY537_SL'); cfg.trg_chn     = 130:136; end
    ...
    for iTS = 1:numel(trl_hld)
        ...
        elseif strcmpi(subj,'NY537_SL') || strcmpi(subj,'NY540_SL')
    ...
    save(['/space/mdeh4/1/halgdev/projects/mmilanguage/SL/epoch_data/clerical/' '/trialfun_output/' subj '.mat'],'trl_hld');
else
    load(['/space/mdeh4/1/halgdev/projects/mmilanguage/SL/epoch_data/clerical' '/trialfun_output/' subj '.mat'],'trl_hld');
end
...
if strcmpi(subj,'NY439_SL')
    data_name = [ones(1,numel(1:2:num_trl)-1) ones(1,numel(2:2:num_trl)-1)*2 ...
...
elseif strcmpi(subj,'NY537_SL') || strcmpi(subj,'NY540_SL')
    data_name = [ones(1,num_trl-1) ones(1,num_trl-1)+num_trl  ones(1,num_trl-1)+num_trl*2
        2:num_trl 2+num_trl:num_trl*2 2+num_trl*2:num_trl*3];
end
...
% Phonemes
if strcmpi(subj,'NY439_SL')
    ipt_num = 1:2:numel(inpath);
elseif strcmpi(subj,'NY537_SL') || strcmpi(subj,'NY540_SL')
    ipt_num = 1:numel(inpath);
end

v_con = []; v_vow = []; a_con = []; a_vow = [];
for iTS = ipt_num % - EJK need to come back and fix, quick hack
    ...
    v_vow = [v_vow ; cell2mat(lst(:,5))+100];
    a_con = [a_con ; cell2mat(lst(:,6))];
    a_vow = [a_vow ; cell2mat(lst(:,7))+100];
...
if  strcmpi(subj,'NY439_SL')    
    ...
    sll_dat.(sll_dat.data_name{1}).cfg.alt_lab.label = ft_correct_channel(chn_loc,sll_dat.(sll_dat.data_name{1}).cfg.alt_lab.label);
    for iA = 2:numel(sll_dat.data_name); sll_dat.(sll_dat.data_name{iA}).cfg.alt_lab.label = sll_dat.(sll_dat.data_name{1}).cfg.alt_lab.label; end
end
end
