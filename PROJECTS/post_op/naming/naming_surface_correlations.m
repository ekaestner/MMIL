...
for iG = 1:numel(cmp_nme)
    for iT = 1:2%numel(cog_col.(cmp_nme{iG}))
        ...
        fcfg.out_pre = [ cog_dta_col{ cog_col.(cmp_nme{iG})(iT)} ];
        
        ejk_surface_correlations( fcfg );
        
        % Check specific ROI correlations
        for iH = 1:2
            
            roi_col = string_find( dta_lbl, [hms_hld{iH}(1:2) '_']);
            if iH == 1; fcfg.dta_hld = fcfg.dta_lhs; elseif iH == 2; fcfg.dta_hld = fcfg.dta_rhs; end
            
            fcfg.hms     = hms_hld{iH}(1:2);
            
            fcfg.rvl_fle = [ out_put '/' cmp_out{iG} '/' cog_dta_col{ cog_col.(cmp_nme{iG})(iT)} '/' 'rvalues' '_' hms_hld{iH} '_' 'dep_var' '_' cmp_nme{iG} '.mat' ];
            
            fcfg.roi_lbl = cellfun( @(x) x(4:end), dta_lbl(roi_col), 'uni', 0);
            fcfg.roi_hld = dta_hld( grp.(cmp_nme{iG}), roi_col );
            
            fcfg.prc_loc = '/home/ekaestner/gitrep/MMIL/EXTERNAL/freesurfer';
            fcfg.prc_nme = '.aparc.annot';
            fcfg.prc_lbl = { 'fusiform' 'inferiortemporal' 'superiortemporal' 'supramarginal' 'parstriangularis'};
                        
            fcfg.out_dir = [ out_put '/' cmp_out{iG} '/' cog_dta_col{ cog_col.(cmp_nme{iG})(iT)} '/' 'check_rois' '/'];
            
            ejk_check_surfaces_roi( fcfg );
