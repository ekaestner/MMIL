...
% DEM_SBJ
dem_sbj_dta =  lod_dta(:,[1 2:4 37:38]);
dem_sbj_sbj_nme = dem_sbj_dta(2:end,1);
dem_sbj_roi_nme = dem_sbj_dta(1,2:end);
dem_sbj_dta     = dem_sbj_dta(2:end,2:end); 

% MEMORY_SCORES
mem_scr_dta =  lod_dta(:,[1 5:12]);
mem_scr_sbj_nme = mem_scr_dta(2:end,1);
mem_scr_roi_nme = mem_scr_dta(1,2:end);
mem_scr_dta     = cell2mat(mem_scr_dta(2:end,2:end)); 

% FA_TRACTS
tfa_trc_dta = lod_dta(:,[1 13:22]); 
tfa_trc_sbj_nme = tfa_trc_dta(2:end,1);
tfa_trc_roi_nme = tfa_trc_dta(1,2:end);
tfa_trc_dta     = cell2mat(tfa_trc_dta(2:end,2:end));  

% FA_WMPARC
tfa_srf_dta =  lod_dta(:,[1 23:32]);
tfa_srf_sbj_nme = tfa_srf_dta(2:end,1);
tfa_srf_roi_nme = tfa_srf_dta(1,2:end);
tfa_srf_dta     = cell2mat(tfa_srf_dta(2:end,2:end));    

% FA_VOL
tfa_vol_dta =  lod_dta(:,[1 33:36]);
tfa_vol_sbj_nme = tfa_vol_dta(2:end,1);
tfa_vol_roi_nme = tfa_vol_dta(1,2:end);
tfa_vol_dta     = cell2mat(tfa_vol_dta(2:end,2:end));  
