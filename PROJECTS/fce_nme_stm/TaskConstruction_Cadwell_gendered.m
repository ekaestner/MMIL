...
ovr_fld = '/home/ekaestner/Dropbox/McDonald Lab/Erik/Methods/DataCollection/Stimulation/Task_v3/'; 
...
for iNL = 1:num_lst+2
    ...
    % Balance Gender selection - Make only males, only females
    if rem(iNL,2)==1
        lst_gen = repmat({'M'},1,num_lst_stm);
    elseif rem(iNL,2)==0
        lst_gen = repmat({'F'},1,num_lst_stm);
    end
...
for iL = 1:numel(lst_nme)
    ...
    for iFS = 1:size(stm_hld,1)
        ...
        % Choose Name        
        nme_use_ind        = randsample(1:size(nme.(nme_idn{iFS}),1),1);
        nme_use(iFS,1)     = upper(nme.(nme_idn{iFS})(nme_use_ind,nme_stm_var_col));
        nme_use_nor(iFS,:) = nme.(nme_idn{iFS})(nme_use_ind,nme_nor_var_col);     
        nme.(nme_idn{iFS})(nme_use_ind,:) = [];
...
%% Make associative recognition lists
% Get list names
lst_nme = dir([ enc_lst_out_fld '/' '*.csv' ]);
lst_nme = {lst_nme(:).name};
lst_nme(string_find(lst_nme,'Remaining')) = [];
...
rcl_lst_nme = lst_nme(string_find(lst_nme,{'m_recall.csv'}));
rcg_lst_nme = lst_nme(string_find(lst_nme,{'m_recognition.csv'}));

pos_chk = 1;
while pos_chk
    tic
    trg_col_ind = [];
    for iL = 1:numel(rcl_lst_nme)
        
        % Load lists
        rcl_lst_hld = mmil_readtext([ enc_lst_out_fld '/' rcl_lst_nme{iL}]);
        rcg_lst_hld = mmil_readtext([ enc_lst_out_fld '/' rcg_lst_nme{iL}]);
        
        % Setup list skeleton
        asc_rcg = [ rcl_lst_hld(:,1:6) cell(size(rcl_lst_hld,1),asc_rcg_num) rcl_lst_hld(:,7:end) ];
        asc_rcg(2:end,3) = repmat({'associative_recognition'},num_lst_stm,1);
        
        nme_chk = 1;
        while nme_chk
            
            % Get name pool
            old_nme = rcl_lst_hld(2:end,2);
            nov_nme = rcg_lst_hld(strcmpi(rcg_lst_hld(:,end),'NOVEL'),2);
            
            % Make names
            nme_pos = cell(numel(old_nme),asc_rcg_num);
            
            unq_num = nan(numel(old_nme),1);
            for iR = 1:numel(old_nme)
                % Choose names
                trg_nme = old_nme{iR};
                old_lre = randsample(old_nme,2)';
                nov_lre = randsample(nov_nme,1);
                nov_nme(strcmpi(nov_nme,nov_lre)) = [];
                
                % Create And Shuffle
                nme_pos(iR,:) = [ trg_nme old_lre nov_lre ];
                nme_pos(iR,:) = nme_pos(iR,randperm(asc_rcg_num));
                
                % Check unique names per row
                unq_num(iR) = numel(unique(nme_pos(iR,:)));
                
            end
            
            % Check each old name appears 3 times
            tbl_chk = tabulate(nme_pos(:));
            nme_app = cell2mat(tbl_chk(ismember(tbl_chk(:,1),old_nme),2));
            
            % Check
            if all(unq_num==asc_rcg_num) && all(nme_app==3)
                nme_chk = 0;
                for iR = 1:numel(old_nme)
                    trg_col_ind = [ trg_col_ind find(strcmpi(nme_pos(iR,:),old_nme{iR}))];
                end
            end
        end
        
        %
        %     asc_rcg(1,7:7+asc_rcg_num-1) = strcat(repmat({'Name'},1,asc_rcg_num),cellfun(@(x) {num2str(x)},num2cell(1:asc_rcg_num)));
        asc_rcg(2:end,7:7+asc_rcg_num-1) = nme_pos;
        cell2csv([ ret_out_fld '/' lst_nme{iL}(1:end-4) '_' 'associative_recognition' '.csv'],asc_rcg)
        
    end
    
    % Final position check
    toc
    pos_tbl = tabulate(trg_col_ind);
    if all(pos_tbl(:,2)>=floor((numel(trg_col_ind)/4)-2) & pos_tbl(:,2)<=ceil((numel(trg_col_ind)/4)+2))
        pos_chk = 0;
    else
        pos_tbl(:,2)'
    end
