...
krs_ele_sel = mmil_readtext('/home/ekaestne/gitrep/MMIL/PROJECTS/XS/SupplFig1_NY441.csv');
...
for iS = 1:numel(srf_sbj_nme)
    
    try
        frs_ele_loc_lhs = mmil_readtext( [ ele_loc '/' srf_sbj_nme '_' 'XS' '/' 'electrode_location' '/' 'NY441_fmri_021414_coor_T1_2014-02-26_edited.txt' ] );
    catch
        frs_ele_loc_lhs = [];
    end
    
    try
        frs_ele_loc_rhs = mmil_readtext( [ ele_loc '/' srf_sbj_nme '_' 'XS' '/' 'electrode_location' '/' 'NY441_fmri_021414_coor_T1_2014-02-26_edited.txt' ] );
    catch
        frs_ele_loc_rhs = [];
    end
    ...
    if ~isempty(frs_ele_loc_lhs)
        tot_ele_lhs = [ tot_ele_lhs ;  [ strcat( srf_sbj_nme , '_' , frs_ele_loc_lhs(:,1)) frs_ele_loc_lhs(:,2:end) ] ];
    end
    
    % rhs
    if ~isempty(frs_ele_loc_rhs)
        tot_ele_rhs = [ tot_ele_rhs ;  [ strcat( srf_sbj_nme , '_' , frs_ele_loc_rhs(:,1)) frs_ele_loc_rhs(:,2:end) ] ];
    end
    ...
    tot_ele = [ tot_ele ;  [ strcat( srf_sbj_nme , '_' , tot_ele_hld(:,1)) tot_ele_hld(:,2:end) ] ];
    
end

%%
% lhs total
cell2csv( [ ele_loc '/' srf_sbj_nme '_' 'XS' '/'  'electrode_location' '/' 'total_lhs_ecog'] , tot_ele_lhs );

% rhs total
cell2csv( [ ele_loc '/' srf_sbj_nme '_' 'XS'  '/'  'electrode_location' '/' 'total_rhs_ecog'] , tot_ele_rhs );
...
cfg.pial_mat  = {[ ele_loc '/' srf_sbj_nme '_' 'XS' '/'  'surf' '/' 'lh.pial']                       [ ele_loc '/' srf_sbj_nme '_' 'XS' '/'  'surf' '/' 'rh.pial']};
cfg.elec_text = {[ ele_loc '/' srf_sbj_nme '_' 'XS' '/'  'electrode_location' '/' 'total_lhs_ecog']  [ ele_loc '/' srf_sbj_nme '_' 'XS' '/'  'electrode_location' '/' 'total_rhs_ecog']};
