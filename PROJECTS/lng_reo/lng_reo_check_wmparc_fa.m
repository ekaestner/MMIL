...
% %%%%%%%%%%%%%%%
wmp_dta = mmil_readtext( [ prj_dir '/' 'OUTPUT' '/' prj_nme '/' 'Data' '/' 'WMParc' '/' 'WMParc_aparc__FA.csv' ] );
...
fcfg.out_dir = [ prj_dir '/' 'OUTPUT' '/' prj_nme '/' 'DataCheck' '/' 'WMParcFA' '/' 'Desikan' '/'];
...
for iRO = 1:size(wmp_lbl,2)
    ...
    fcfg.out_dir = [ prj_dir '/' 'OUTPUT' '/' prj_nme '/' 'DataCheck' '/' 'WMParcFA' '/' 'Desikan' '/' 'Activation'];
...
for iRO = 1:size(wmp_lbl,2)
    ...
    if ~ (all(isnan(lft_roi)) && all(isnan(rgh_roi)) && all(isnan(con_roi)))
        ...
        fcfg.out_dir = [ prj_dir '/' 'OUTPUT' '/' prj_nme '/' 'DataCheck' '/' 'WMParcFA' '/' 'Desikan' '/' 'Activation_zscore'];
