...
function ft_Utah_trialfunMG67SA(data,trl)
...
pos = 1.5*Fs;
%% Find the timings
tmpdat = dat;    tmpdat([1:4e5])=0;   tmpdat([1.04e6:end])=0;
...
for i=1:length(evt)
 ...
 m1 = [mtx(141,1)+1100  mtx(141,2)+1100 -250 0];
 m2 = [mtx(155,1)+1100  mtx(155,2)+1100 -250 0];
 m3 = [mtx(208,1)+1100  mtx(208,2)+1100 -250 0];
 
trl = [mtx(1:141,:); m1; mtx(142:155,:); m2; mtx(156:208,:); m3; mtx(209:end,:)];
trl(:,4) = evt_val(1:480);
