...
indir   = {'/space/mdeh3/7/halgdev/analysis/iEEG_NYU/NY08/NY08_SA/eeg' ...
           '/space/mdeh3/7/halgdev/analysis/iEEG_NYU/NY08/NY08_SZ/eeg'};
...
cfg.task   = 'SA';
cfg.inpath = inpath{1};
skp{1}     = mmil_check_eeg(cfg);

cfg        = [];
cfg.task   = 'SZ';
cfg.inpath = inpath{2};
skp{2}     = mmil_check_eeg(cfg);

cfg          = [];
cfg.task     = {'SA' 'SZ'};
cfg.skp      = skp;
cfg.specific = {'task' 'skp'; [1 2] [1 2]};
...
% cfg.new_events  = num2cell(cellfun(@(x) x+10,num2cell([1:8])));
...
rmv_chn = [rmv_chn numel(sem_dat.(sem_dat.data_name{1}).label)-3:numel(sem_dat.(sem_dat.data_name{1}).label)];
rmv_chn = [rmv_chn 1:48];

for iFX = 1:numel(sem_dat.(sem_dat.data_name{2}).label); 
    spc_ind = strfind(sem_dat.(sem_dat.data_name{2}).label{iFX},' ');
    sem_dat.(sem_dat.data_name{2}).label{iFX} = sem_dat.(sem_dat.data_name{2}).label{iFX}(1:spc_ind-1);
end
...
% chn_nse_grp = {[1:numel(sem_dat.(sem_dat.data_name{1}).label)]};
...
cfg = [];
cfg.return_events = 0;
cfg.old_events  = num2cell([1:8 11:18]);
cfg.new_events  = num2cell([1:8 11:18]);
cfg.crt_alt_eve = 'trialinfo';
sem_dat = ft_func(@ft_redefine_events,cfg,sem_dat);

cfg = [];
cfg.return_events = 0;
cfg.old_events  = {1:8 11:18};
...
cfg.nse_tme     = [-0.4 0];
cfg.sig_tme     = [0 0.8];
cfg.alt_eve_loc = {'ovr_all_evt' 'ovr_all_evt'};
cfg.eve         = {101 102};
cfg.snr_lab     = {'vis' 'aud'};
