function iSASZ_initial_events(fcfg)
...
if any(strcmpi(tsk,'sz'))
    ...
    % ani/obj priming
    dat_ind = find(strcmpi(tsk,'sz'));
    
    for iD = 1:numel(dat_ind)
        
        for iW = 1:numel(eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj)-1
            if eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW)==eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW+1)
                if eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==111 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==111
                    typ(iW) = 1;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==112 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==111
                    typ(iW) = 2;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==111 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==112
                    typ(iW) = 3;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==112 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==112
                    typ(iW) = 4;
                end
            elseif  eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW)~=eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW+1)
                typ(iW) = 999;
                if eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==111 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==111
                    typ(iW) = 5;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==112 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==111
        ...
        eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.cat_prm = [0 ; typ'];
    ...
    cfg.data_name   = find(strcmpi(tsk,'sz'));
    cfg.eve_rul     = {'new' 'new'};
    cfg.old_events  = {{[1 2 3 4] [5 6 7 8]} {1 5}};
    cfg.new_events  = {[1001 1002]           [1011 1012]};
    cfg.crt_alt_eve = {'cat_prm_all' 'cat_prm_new'};
    cfg.use_alt_eve = {'cat_prm'     'cat_prm'};
    eve_dat = ft_func(@ft_redefine_events2,cfg,eve_dat);
...
if any(strcmpi(tsk,'sa'))
    ...
    % ani/obj priming
    dat_ind = find(strcmpi(tsk,'sa'));
    
    for iD = 1:numel(dat_ind)
        
        for iW = 1:numel(eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj)-1
            if eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW)==eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW+1)
                if eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==211 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==211
                    typ(iW) = 11;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==212 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==211
                    typ(iW) = 12;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==211 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==212
                    typ(iW) = 13;
                elseif eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW)==212 && eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.new_old(iW+1)==212
                    typ(iW) = 14;
                end
            elseif  eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW)~=eve_dat.(eve_dat.data_name{dat_ind(iD)}).cfg.alt_eve.ani_obj(iW+1)
                typ(iW) = 999;
    ...
    cfg.data_name   = find(strcmpi(tsk,'sa'));
    cfg.eve_rul     = {'new' 'new'};
    cfg.old_events  = {{[11 12 13 14] [15 16 17 18]} {11 15}};
    cfg.new_events  = {[2001 2002]                   [2011 2012]};
    cfg.crt_alt_eve = {'cat_prm_all'                 'cat_prm_new'};
    cfg.use_alt_eve = {'cat_prm'                     'cat_prm'};
    eve_dat = ft_func(@ft_redefine_events2,cfg,eve_dat);
