function FW_analysis(fcfg)

fprintf([fcfg.sbj_nme ': Starting initial analysis work on %s \n'],fcfg.sbj_nme)

%% Responsive Effects
cfg = [];

cfg.typ      = { 'lfp' 'hgp' };
cfg.ele_typ  = { 'ecog' };
cfg.loc_typ  = { 'split' };

cfg.cmb_nme = { 'eff_600' };

cfg.sig_loc = { { 'vis_stm' } };
           
cfg.sig_chn = { [ 1 ] };

% Table Combination
cfg.run_tbl     = [1];

cfg.tbl_cmb_nme = { { 'Visual Language' } };
               
cfg.tbl_cmb_rul = { { '1' } };
               
cfg.tbl_sum = { [] };

% Plot combination
cfg.run_plt     = [1];

cfg.plt_cmb_nme = { { 'Visual Language' } };
               
cfg.plt_cmb_col = { { 'maroon' } };
               
cfg.plt_cmb_rul     = { { '1' } };
cfg.plt_prb_cmb_rul = { { '1' } };

cfg.plt_prb_cmb_max = { [ 1 ] };

% First Pass combinations
cfg.run_fst     = [ 1 ];

cfg.fst_cmb_nme = { { 'Visual Language' } };
               
cfg.fst_cmb_col = { { 'maroon' } };
               
cfg.fst_cmb_rul = { { '1' } };
               
cfg.fst_ord     = {{}};
cfg.fst_ord_dim = {{'avg'} };   
cfg.fst_ord_nme = {{}};   

% Time combinations
cfg.run_tme     = [0];

cfg.tme_cmb_nme = { { 'Visual Language' } };
               
cfg.tme_cmb_col = { { 'maroon' } };
               
cfg.tme_cmb_rul = { { '1' } };

cfg.tme_ord     = { { 1 } };
cfg.tme_ord_nme = { { 'vis' } };   
    
% Line Plot combinations
cfg.run_lne     = [1];

cfg.plt_dim = [ 1 1 ];
cfg.dat_loc = [ 1 ];

cfg.alt_lab = { 'label' };

cfg.alt_eve = { { 'trialinfo' } };
cfg.eve     = { { [3 4 5 6] } };
cfg.lnstyle.col_ord = { { { rgb('bright red')   rgb('orange') rgb('dark red')   rgb('reddish grey')} } };
                     
cfg.stt_dat = { { {'vis_stm'} } };
cfg.stt_col = { { { ft_stt_col(rgb('reddish gray')) } } };

cfg.v_lne       = { [0 0.2 0.4] };
cfg.v_lne_col   = { {rgb('red') rgb('black') rgb('black')} };   
                
% Run
cfg.dat_nme     = '_overall_data';
cfg.sbj_clr_fld = fcfg.clr_fld;
cfg.sbj_nme = fcfg.sbj_nme;

cfg.fle_out_pth = fcfg.dat_fld;

mmil_combo_effects(cfg);

% Save for overall analysis
for iC = 1:numel(cfg.cmb_nme); cfg.clr_fld = fcfg.clr_fld; cfg.iC = iC; mmil_ovr_ana_sve(cfg) ; end

%% Letter/False Font Effects
cfg = [];

cfg.typ      = { 'lfp' 'hgp' };
cfg.ele_typ  = { 'ecog' };
cfg.loc_typ  = { 'split' };

cfg.cmb_nme = { 'ltr_ffn_600' };

cfg.sig_loc = { { 'vis_stm' 'vis_ltr_msk' 'vis_ltr_msk' } };
           
cfg.sig_chn = {[1 1 2]};

% Table Combination
cfg.run_tbl     = [1];

cfg.tbl_cmb_nme = { { 'Letter Selective' 'False Font Selective' } };
               
cfg.tbl_cmb_rul = { { '1 & 2' '1 & 3' } };
               
cfg.tbl_sum = {[]};

% Plot combination
cfg.run_plt     = [1];

cfg.plt_cmb_nme = { { 'Letter Selective' 'False Font Selective' } };
               
cfg.plt_cmb_col = { { 'dark red'   'reddish grey' } };
               
cfg.plt_cmb_rul     = { { '1 & 2' '1 & 3' } };
cfg.plt_prb_cmb_rul = { { '1 & 2' '1 & 3' } };

cfg.plt_prb_cmb_max = {[1 1]};

% First Pass combinations
cfg.run_fst     = [ 1 ];

cfg.fst_cmb_nme = { { 'Letter Selective' 'Voice Selective' } };
               
cfg.fst_cmb_col = { { 'dark red' 'reddish grey' } };
               
cfg.fst_cmb_rul = { { '2' '3' } };
               
cfg.fst_ord     = {{}};
cfg.fst_ord_dim = {{'avg'} };   
cfg.fst_ord_nme = {{}};   

% Time combinations
cfg.run_tme     = [0];

cfg.tme_cmb_nme = { { 'Letter Selective' 'Voice Selective' } };
               
cfg.tme_cmb_col = { { 'dark red' 'reddish grey' } };
               
cfg.tme_cmb_rul = {{'1' '2'} };

cfg.tme_ord     = { { 1 } };
cfg.tme_ord_nme = { { 'vis' } };   
    
% Line Plot combinations
cfg.run_lne     = [1];

cfg.plt_dim = [2 2];
cfg.dat_loc = [1 1 ; 1 1];

cfg.alt_lab = { 'label' };

cfg.alt_eve = { {'trialinfo' 'trialinfo' ; 'trialinfo' 'trialinfo' } };
cfg.eve     = { {[3 4 5 6] [5 6]; [3 5] [3 4]} };
cfg.lnstyle.col_ord = { { { rgb('bright red')   rgb('orange') rgb('dark red')   rgb('reddish grey')} {rgb('dark red')   rgb('reddish grey')} ; {rgb('bright red')   rgb('dark red')} {rgb('bright red')   rgb('orange')} } };
                     
cfg.stt_dat = { {{'vis_stm'} {'vis_ltr_msk'} ; {'vis_wrd_msk'} {'vis_old_msk'} } };
cfg.stt_col = { {{ft_stt_col(rgb('reddish gray'))} {ft_stt_col(rgb('dark red'))} ; {ft_stt_col(rgb('bright red'))} {ft_stt_col(rgb('orange'))} } };

cfg.v_lne       = { [0 0.2 0.4] };
cfg.v_lne_col   = { {rgb('red') rgb('black') rgb('black')} };   
                
% Run
cfg.dat_nme     = '_overall_data';
cfg.sbj_clr_fld = fcfg.clr_fld;
cfg.sbj_nme = fcfg.sbj_nme;

cfg.fle_out_pth = fcfg.dat_fld;

mmil_combo_effects(cfg);

% Save for overall analysis
for iC = 1:numel(cfg.cmb_nme); cfg.clr_fld = fcfg.clr_fld; cfg.iC = iC; mmil_ovr_ana_sve(cfg) ; end

%% Word/Non-Word Effects
cfg = [];

cfg.typ      = { 'lfp' 'hgp' };
cfg.ele_typ  = { 'ecog' };
cfg.loc_typ  = { 'split' };

cfg.cmb_nme = { 'wrd_nwd_600' };

cfg.sig_loc = { { 'vis_stm' 'vis_wrd_msk' 'vis_wrd_msk' } };
           
cfg.sig_chn = {[1 1 2]};

% Table Combination
cfg.run_tbl     = [1];

cfg.tbl_cmb_nme = { { 'Word Selective' 'Non-Word Selective' } };
               
cfg.tbl_cmb_rul = { { '1 & 2' '1 & 3' } };
               
cfg.tbl_sum = {[]};

% Plot combination
cfg.run_plt     = [1];

cfg.plt_cmb_nme = { { 'Word Selective' 'Non-Word Selective' } };
               
cfg.plt_cmb_col = { { 'bright red'   'dark red' } };
               
cfg.plt_cmb_rul     = { { '1 & 2' '1 & 3' } };
cfg.plt_prb_cmb_rul = { { '1 & 2' '1 & 3' } };

cfg.plt_prb_cmb_max = {[1 1]};

% First Pass combinations
cfg.run_fst     = [ 1 ];

cfg.fst_cmb_nme = { { 'Word Selective' 'Non-Word Selective' } };
               
cfg.fst_cmb_col = { { 'bright red'   'dark red' } };
               
cfg.fst_cmb_rul = { { '2' '3' } };
               
cfg.fst_ord     = {{}};
cfg.fst_ord_dim = {{'avg'} };   
cfg.fst_ord_nme = {{}};   

% Time combinations
cfg.run_tme     = [0];

cfg.tme_cmb_nme = { { 'Word Selective' 'Non-Word Selective' } };
               
cfg.tme_cmb_col = { { 'bright red'   'dark red' } };
               
cfg.tme_cmb_rul = {{'1' '2'} };

cfg.tme_ord     = { { 1 } };
cfg.tme_ord_nme = { { 'vis' } };   
    
% Line Plot combinations
cfg.run_lne     = [1];

cfg.plt_dim = [2 2];
cfg.dat_loc = [1 1 ; 1 1];

cfg.alt_lab = { 'label' };

cfg.alt_eve = { {'trialinfo' 'trialinfo' ; 'trialinfo' 'trialinfo' } };
cfg.eve     = { {[3 4 5 6] [5 6]; [3 5] [3 4]} };
cfg.lnstyle.col_ord = { { { rgb('bright red')   rgb('orange') rgb('dark red')   rgb('reddish grey')} {rgb('dark red')   rgb('reddish grey')} ; {rgb('bright red')   rgb('dark red')} {rgb('bright red')   rgb('orange')} } };
                     
cfg.stt_dat = { {{'vis_stm'} {'vis_ltr_msk'} ; {'vis_wrd_msk'} {'vis_old_msk'} } };
cfg.stt_col = { {{ft_stt_col(rgb('reddish gray'))} {ft_stt_col(rgb('dark red'))} ; {ft_stt_col(rgb('bright red'))} {ft_stt_col(rgb('orange'))} } };

cfg.v_lne       = { [0 0.2 0.4] };
cfg.v_lne_col   = { {rgb('red') rgb('black') rgb('black')} };   
                
% Run
cfg.dat_nme     = '_overall_data';
cfg.sbj_clr_fld = fcfg.clr_fld;
cfg.sbj_nme = fcfg.sbj_nme;

cfg.fle_out_pth = fcfg.dat_fld;

mmil_combo_effects(cfg);

% Save for overall analysis
for iC = 1:numel(cfg.cmb_nme); cfg.clr_fld = fcfg.clr_fld; cfg.iC = iC; mmil_ovr_ana_sve(cfg) ; end

%% Novel/Repetition Effects
cfg = [];

cfg.typ      = { 'lfp' 'hgp' };
cfg.ele_typ  = { 'ecog' };
cfg.loc_typ  = { 'split' };

cfg.cmb_nme = { 'nov_rep_600' };

cfg.sig_loc = { { 'vis_stm' 'vis_old_msk' 'vis_old_msk' } };
           
cfg.sig_chn = {[1 1 2]};

% Table Combination
cfg.run_tbl     = [1];

cfg.tbl_cmb_nme = { { 'Novel Selective' 'Repetition Selective' } };
               
cfg.tbl_cmb_rul = { { '1 & 2' '1 & 3' } };
               
cfg.tbl_sum = {[]};

% Plot combination
cfg.run_plt     = [1];

cfg.plt_cmb_nme = { { 'Novel Selective' 'Repetition Selective' } };
               
cfg.plt_cmb_col = { { 'bright red'   'orange' } };
               
cfg.plt_cmb_rul     = { { '1 & 2' '1 & 3' } };
cfg.plt_prb_cmb_rul = { { '1 & 2' '1 & 3' } };

cfg.plt_prb_cmb_max = {[1 1]};

% First Pass combinations
cfg.run_fst     = [ 1 ];

cfg.fst_cmb_nme = { { 'Novel Selective' 'Repetition Selective' } };
               
cfg.fst_cmb_col = { { 'bright red'   'orange' } };
               
cfg.fst_cmb_rul = { { '2' '3' } };
               
cfg.fst_ord     = {{}};
cfg.fst_ord_dim = {{'avg'} };   
cfg.fst_ord_nme = {{}};   

% Time combinations
cfg.run_tme     = [0];

cfg.tme_cmb_nme = { { 'Novel Selective' 'Repetition Selective' } };
               
cfg.tme_cmb_col = { { 'bright red'   'orange' } };
               
cfg.tme_cmb_rul = {{'1' '2'} };

cfg.tme_ord     = { { 1 } };
cfg.tme_ord_nme = { { 'vis' } };   
    
% Line Plot combinations
cfg.run_lne     = [1];

cfg.plt_dim = [2 2];
cfg.dat_loc = [1 1 ; 1 1];

cfg.alt_lab = { 'label' };

cfg.alt_eve = { {'trialinfo' 'trialinfo' ; 'trialinfo' 'trialinfo' } };
cfg.eve     = { {[3 4 5 6] [5 6]; [3 5] [3 4]} };
cfg.lnstyle.col_ord = { { { rgb('bright red')   rgb('orange') rgb('dark red')   rgb('reddish grey')} {rgb('dark red')   rgb('reddish grey')} ; {rgb('bright red')   rgb('dark red')} {rgb('bright red')   rgb('orange')} } };
                     
cfg.stt_dat = { {{'vis_stm'} {'vis_ltr_msk'} ; {'vis_wrd_msk'} {'vis_old_msk'} } };
cfg.stt_col = { {{ft_stt_col(rgb('reddish gray'))} {ft_stt_col(rgb('dark red'))} ; {ft_stt_col(rgb('bright red'))} {ft_stt_col(rgb('orange'))} } };

cfg.v_lne       = { [0 0.2 0.4] };
cfg.v_lne_col   = { {rgb('red') rgb('black') rgb('black')} };   
                
% Run
cfg.dat_nme     = '_overall_data';
cfg.sbj_clr_fld = fcfg.clr_fld;
cfg.sbj_nme = fcfg.sbj_nme;

cfg.fle_out_pth = fcfg.dat_fld;

mmil_combo_effects(cfg);

% Save for overall analysis
for iC = 1:numel(cfg.cmb_nme); cfg.clr_fld = fcfg.clr_fld; cfg.iC = iC; mmil_ovr_ana_sve(cfg) ; end

end